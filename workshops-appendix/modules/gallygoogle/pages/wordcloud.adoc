...
// dead-link: https://the-examples-book.com/data-viz/_attachments/visual_vocabulary.pdf[The link]
...
Note that in xref:tools:python:variables.adoc#bool[bool], `True` has an equivalent value of 1, and `False` has an equivalent value of 0. 
...
The code above consists multiple lines and can be shortened to a line as shown below using a xref:tools:python:lists.adoc#list-comprehensions[list comprehension].
...
Because the function, `WordCloud`, only accepts xref:tools:python:variables.adoc#str[`string`] format as argument, we can simply combine the whole `Text` column into a long string.
