...
_Recall that in xref:tools:python:variables.adoc#bool[bool], `False` has an equivalent value of `0`, and `True` has an equivalent value of `1`._
...
We can add a new column to our dataframe representing the number of tags found for each video using a xref:tools:python:lists.adoc#list-comprehensions[list comprehension].
