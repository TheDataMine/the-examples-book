...
Remember, in Python, you can use `**` to raise a number to a power. For example $2^3$ would be `2**3`.
====
...
$\hat{y}_h \pm t_{(\alpha/2, n-p)}*\sqrt{MSE*diag(x_h(X^TX)^{-1}x_h^T)}$
...
$\hat{Y} \pm 1.993464*\sqrt{MSE*diag(X(X^TX)^{-1}X^T})$
