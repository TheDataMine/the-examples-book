...
SLURM is a job scheduler found on about 60% of the top 500 supercomputers.footnote:[https://en.wikipedia.org/wiki/Slurm_Workload_Manager[https://en.wikipedia.org/wiki/Slurm_Workload_Manager]] In this project (and the next) we will learn about ways to schedule jobs on SLURM, and learn the tools used. n

Let's get started by using a script called `sinteractive` written by Lev Gorenstein, here at Purdue. A breif explanation is that `sinteractive` gets some resources (think memory and cpus), and logs you into that "virtual" system. 
...
Reading the explanation from SLURM's website is not enough to understand, running the configurations will help your understanding. If you have simple, parallel processes, that doesn't need to have any shared state, you can use a  single `srun` per task. Each with `--mem-per-cpu` (so memory availability is more predictable), `-n 1`, followed by `&` ( just a reminder that `&` at the end of a bash command puts the process in the background).

Reading the information about cgroups may lead you to wonderinf if the RCAC puts you into a cgroup when you are SSH'd into a frontend. Use our `get_info.py` script, along with other unix commands, to determine if you are in a cgroup. If you are in a cgroup, how many cpus and memory do you have?
...
2. Write 1-2 sentences of any observations 
...
It is time consuming and not best practice to manually go thru the entire dataset to find the duplicate. Thinking back to some of the past work, we remember that a hash algorithm is a good way to identify the duplicate image. 

Below is the hash you can load and use 
