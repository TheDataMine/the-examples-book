...
For station with `station_id` 25, get a vector containing all `temperature` >= 85. Call this vector `hot_temps`. Get a vector containing all `temperature` <= 40, and call this vector `cold_temps`. How many elements are in `hot_temps`? How many elements are in `cold_temps`? Attempt to add the vectors together. What happens? Read https://excelkingdom.blogspot.com/2018/01/what-recycling-of-vector-elements-in-r.html[this] to understand what is happening.
