...
The reason they are benifical is that Ksuids are sortable by time and unique identifers (there is a minimal chane that any two id's would be the same). 
...
You can even put your `degrees_to_radians` function in the `get_distance` function. Any "nested" (a function within a function)function can be called a "helper" function. If you have code that will be use multiple times it is benificial to create a "helper" function will be use multiple times. 
