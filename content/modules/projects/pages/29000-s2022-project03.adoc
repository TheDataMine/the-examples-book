...
First, in your browser, type in `34474` into the search bar and press enter/return. There are two possible outcomes of this search, depending on the computer you are using and whether or not you've been browsing zillow. The first is your search results. The second is the page shown below, where the user is asked to select which type of listing they would like to see.

image::figure01.webp[, width=792, height=500, loading=lazy, title="some_title"]
