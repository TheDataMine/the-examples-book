...
srun --exclusive -n 1 --mem-per-cpu=1000 -t 00:00:00 $HOME/gi.py > 1.txt &
srun --exclusive -n 1 --mem-per-cpu=1000 -t 00:00:00 $HOME/gi.py > 2.txt &
srun --exclusive -n 1 --mem-per-cpu=1000 -t 00:00:00 $HOME/gi.py > 3.txt &
srun --exclusive -n 1 --mem-per-cpu=1000 -t 00:00:00 $HOME/gi.py > 4.txt &
...
[IMPORTANT]
====
In order for the `imagehash` library to work, we need to make sure the libffi dependency is loaded up. Before executing the hash script in your `srun` command, first prepend `module use /scratch/brown/kamstut/tdm/opt/modulefiles; module load libffi/3.4.2;`.
