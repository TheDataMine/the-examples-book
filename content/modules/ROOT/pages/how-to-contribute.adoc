...
ROOT::
The `ROOT` module. This is the default, required module. No further modules are required. An appendix or core book may not need more than the `ROOT` module. In fact, our https://github.com/TheDataMine/example-appendix[example appendix] only contains the `ROOT` module.
...
pages::
A "family directory" that should be used to store each page of the book. These are AsciiDoc files. This is where the actual content lives. [[page-naming]]In this book, each `.adoc` file should be named the same as the page title, in all lowercase, with spaces replaced by hyphens. For example, a page with the title "Introduction to Python" should be named `introduction-to-python.adoc`.
...
Adding an appendix to the book is straightforward. 
...
----
+
. Create and place the desired AsciiDoc content in the `pages` directory. Make sure to follow our <<page-naming, page naming convention>>.
...
+
[TIP]
Check out this book's https://github.com/TheDataMine/the-examples-book/blob/staging/antora-playbook.yml[`antora-playbook.yml`] file and https://github.com/TheDataMine/example-appendix[example appendix] for more details.
