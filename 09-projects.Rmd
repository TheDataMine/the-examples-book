...
##### 2. Write a function called `get_knn` that accepts the `cosine_similarity_matrix`, a `username`, and a value, `k`. The function `get_knn` should return a `pandas` Series containing the usernames of the `k` most similar users to the input `username`.
...
print([round(num, 3) for num in k_similar.tolist()]) # [0.916, 0.912, 0.912, 0.91]
```
...
**Relevant topics:** [ggplot](), [geom_point](), [aes](), [labs]()
