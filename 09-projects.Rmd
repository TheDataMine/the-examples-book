...
##### 2. Write a function called `get_knn` that accepts the `cosine_similarity_matrix`, `username`, and a value, `k`. `get_knn` should return a `pandas` Series containing the `username`s of the `k` most similar users.
...
##### 4. We are so close, and things are looking good! The next step for our system, is to write a function that finds recommendations for a given user. Write a function called `recommend_beers`, that accepts three arguments: the `train` DataFrame, a `username`, a `cosine_similarity_matrix`, and `k` (how many neighbors to use). `recommend_beers` should return the top 5 recommendations.
...
```{python, eval=F}
recommend_beers(train, "22Blue", cosine_similarity_matrix, 30) # [62397, 61100, 110635, 87246, 145155]
```
