...
```{block, type="solution"}
- photos contains photo_id of a photo in the photos folder, a business id, caption and label.
- businesses contains business id with a bunch of info about the business like location and attributes.
- checkins contains a business id and datetime
- tips contains user id, business id, text, datetime, and compliment count
- users contains userid, name, a list of friends user ids, and information abou their yelp usage
- reviews contains a review id, user id, business id, datetime, text of the review and some other basic info.
```
...
```{python, eval=F, class.source="solution"}
def has_attributes(business_id: str) -> bool:
    """
    has_attributes takes a business id and returns
    True if the business has attributes and False otherwise.

    Args:
        business_id (str): Yelp business id.

    Returns:
        bool: Whether or not a business has attributes.
    """
    return ~businesses.loc[businesses.loc[:, "business_id"]==business_id,"attributes"].isna().values[0]


print(has_attributes('f9NumwFMBDn751xgFiRbNA'))
print(has_attributes('XNoUzKckATkOD1hP6vghZg'))
print(has_attributes('Yzvjg0SayhoZgCljUJRF9Q'))
print(has_attributes('7uYJJpwORUbCirC1mz8n9Q'))
```
...
def fix_businesses_data(data_path: str, output_dir: str) -> None:
    """
    fix_data accepts a parquet file that contains data in a specific format. 
    fix_data "explodes" the attributes and hours columns into 39+7 new 
    columns.
    ...
    # unnest the attributes
    businesses = pd.concat([businesses.drop(columns=["attributes"]), businesses.loc[:, "attributes"].apply(pd.Series)], axis=1)
    
    # unnest the hours
    businesses = pd.concat([businesses.drop(columns=["hours"]), businesses.loc[:, "hours"].apply(pd.Series)], axis=1)
...
def unnest(myDF: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
    """
    Given a pandas DataFrame, and a list of column names,
    unnest unnests the columns with the provided column names.

    Args:
        myDF (pandas.core.frame.DataFrame): A pandas DataFrame with 1 or more columns that need to be unnested.
        columns (List[str]): A list containing names of columns to be unnested.

    Returns:
        pandas.core.frame.DataFrame: The resulting pandas DataFrame with unnested columns.
    """
    for column in columns:
        myDF = pd.concat([myDF.drop(columns=[column]), myDF.loc[:, column].apply(pd.Series)], axis=1)
...
def unnest(myDF: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
    ...
    """
    for column in columns:
        if column in myDF.columns and type(myDF.loc[:, column].iloc[0])==type(dict()):
            myDF = pd.concat([myDF.drop(columns=[column]), myDF.loc[:, column].apply(pd.Series)], axis=1)
        else:
            continue
    return myDF
...
##### 1. The company you work for is assigning you to the task of building out some functions for the new API they've built. For simplicity assume you have access to the following `pandas` DataFrame's at all times:
...
**Hint:** You should get the same average number of starts value for the following code:
```{python, eval=F}
...
records = [r.attrib.get("creationDate") for r in records]
records = pd.Series(records)
records = pd.to_datetime(records)
records = records.reset_index()
records.rename(columns={0: 'datetime'}, inplace=True)
records['date'] = records['datetime'].dt.date
records = records.set_index("index")

records_by_day = records.groupby("date").count()
records_by_day = records_by_day.reset_index()
records_by_day.rename(columns={'datetime': 'count'}, inplace=True)

import matplotlib.pyplot as plt

plt.bar(records_by_day['date'], records_by_day['count'])
...
```{block, type="solution"}
869819 records in the time period
```

```{python, eval=F, class.source="solution"}
import datetime
records_by_day['date'] = pd.to_datetime(records_by_day['date'])
records_by_day = records_by_day.loc[(records_by_day['date'].dt.date <= datetime.date(2019, 5, 31)) & (records_by_day['date'].dt.date >= datetime.date(2017, 9, 1)), :]
...
```{python, eval=F, class.source="solution"}
tmp = records.loc[(records['datetime'].dt.date <= datetime.date(2019, 5, 31)) & (records['datetime'].dt.date >= datetime.date(2017, 9, 1)), :]
tmp['dayofweek'] = tmp['datetime'].dt.dayofweek

records_by_dow = tmp.groupby("dayofweek").count()
records_by_dow.rename(columns = {'datetime': 'count'}, inplace=True)
records_by_dow = records_by_dow.reset_index()

labels = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
plt.bar(records_by_dow['dayofweek'], records_by_dow['count'], tick_label=labels)
plt.locator_params(axis='x', nbins=10)
plt.xticks(rotation='vertical')
plt.subplots_adjust(bottom=0.35)
plt.show()
...
myDF = pd.DataFrame(columns=['bpm', 'time', 'date'])
for r in records:
    if r.attrib.get("type") == "HKQuantityTypeIdentifierHeartRateVariabilitySDNN":
        date = r.attrib.get("creationDate")
        for e in r.xpath(".//InstantaneousBeatsPerMinute"):
            myDF = myDF.append({'bpm': e.attrib.get("bpm"), 'date': date, 'time': e.attrib.get("time")}, ignore_index=True)

myDF['date'] = pd.to_datetime(myDF['date']).dt.date
myDF['dayofweek'] = pd.to_datetime(myDF['date']).dt.dayofweek
myDF['dayofweek'] = myDF['dayofweek'].map({0:"Mon", 1:"Tue", 2:"Wed", 3:"Thu", 4:"Fri", 5: "Sat", 6: "Sun"})
```
...
```{python, eval=F, class.source="solution"}
times = pd.to_datetime(myDF.time)
myDF['bpm'] = pd.to_numeric(myDF['bpm'])
to_pivot = myDF.groupby(["dayofweek", times.dt.hour]).mean().reset_index()
to_plot = to_pivot.pivot(index="dayofweek", columns="time", values="bpm")
```

```{python, eval=F, class.source="solution"}
import seaborn as sns

colors = ['#00d4ff', '#0095da', '#0062b6', '#003891', '#001a6d', '#000748', '#020024']
labels = [datetime.datetime.strptime(str(t), "%H").strftime("%H:%M") for t in to_plot.columns]
sns.heatmap(to_plot, cmap=colors, xticklabels=labels)
plt.show()
plt.close()
```
