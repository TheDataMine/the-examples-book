...
##### 1. Write a function called `prepare_data` that accepts an argument called `myDF` that is a `pandas` DataFrame. In addition, `prepare_data` should accept an argument called `min_num_reviews` that is an integer representing the minimum amount of reviews a user or beer must have to be included in the data. `prepare_data` should then return a `pandas` DataFrames. Make sure to remove all rows from `myDF` where a user or beer_id with less than `min_num_reviews`. In addition, remove all rows where `score` is missing (`isna()`). The following should work:
...
**Hint:** Make sure to round up when calculating the number of rows the percentage represents.
...
##### Or if you don't live in the tidyverse:
...
##### Perform the equivalent operations to `cosine_similarity_matrix` using the same technique shown above. Create a new column `standardized_score`. Calculate the `standardized_score` by taking the score and subtracting the average score, then divide by the standard deviation. As it may take a minute or two to create this new column, feel free to test it on a small sample of the reviews DataFrame:
...
positional arguments:
  {stations,cc,daily}  possible commands
    ...
    daily              list data fro a given day and station
...
````
[{'id': 142, 'latitude': 40.10483, 'longitude': -86.86619, 'name': 'WHIN052-MONT004'}, {'id': 150, 'latitude': 41.00467, 'longitude': -86.68428, 'name': 'WHIN058-PULA007'}]
````
