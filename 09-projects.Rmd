...
##### 6. After some thought, your boss decided that using the function from question (5) would get pretty tedious when there are a lot of businesses to include in the plot. You disagree. You think there is a way to pass a list of `business_id`s _without_ modifying your function. Demonstrate how to do this with the list provided:
...
##### Create a Python script called `my_script.py` in your `$HOME` directory. Use the second shebang from the article: `#!/usr/bin/env python`. When run, `my_script.py` should use the `sys` package to print the location of the interpreter being used to run the script. For example, if we started a Python interpreter in RStudio using the following code:
...
##### 3. Okay, great. We now understand that if we want to use packages from a specific environment, we need to modify our shebang accordingly. As it currently stands, our script is pretty useless. Modify the script to accept a single argument. This argument should be a sentence. Your script should then print the sentence, and whether or not the sentence is "POSITIVE" or "NEGATIVE". Use `sys.argv` to accomplish this. Make sure the script functions in the following way:

```{bash, eval=F}
$HOME/my_script.py This is a happy sentence, yay!
```
...
````
./my_script.py requires at least 1 argument, "sentence".
````
...
```{bash, eval=F}
$HOME/my_script.py --score 'This is a happy sentence, yay!'
```
...
```{bash, eval=F}
$HOME/my_script.py -s 'This is a happy sentence, yay!'
```
...
```{bash, eval=F}
$HOME/my_script.py 'This is a happy sentence, yay!' -s
```
...
```{bash, eval=F}
$HOME/my_script.py 'This is a happy sentence, yay!' --score
```
...
```{bash, eval=F}
$HOME/my_script.py 'This is a happy sentence, yay!' --value
```
...
```{bash, eval=F}
$HOME/my_script.py 'This is a happy sentence, yay!' --value --score
```
...
```{bash, eval=F}
$HOME/my_script.py --score
```

````
./my_script.py requires at least 1 argument, "sentence". No sentence provided.
````

```{bash, eval=F}
$HOME/my_script.py 'This is one sentence' 'This is another'
```

````
./my_script.py requires only 1 sentence, but 2 were provided.
````
...
````
usage: my_script.py [-h] [-s] sentence
my_script.py: error: unrecognized arguments: --value
````
...
````
usage: my_script.py [-h] [-s] sentence
my_script.py: error: too few arguments
````
...
````
usage: my_script.py [-h] [-s] sentence
my_script.py: error: unrecognized arguments: This is another
````
