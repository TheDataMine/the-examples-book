...
for mydate in ['06/23/2017', '06/25/2017', '06/30/2017']:
    for myride in ['dinosaur', '7_dwarfs_train', 'soarin']:
        print(f'On the day {mydate} there were a total of', getdisneyrides(mydate, myride), 'rides on the {myride}')
----
...
and now we wrap this work into a function called `myridesreport`:

[source,python]
----
PUT THE WORK FROM THE FUNCTION HERE
----
