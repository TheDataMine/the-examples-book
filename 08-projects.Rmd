...
fun_plot <- function(my_authors_dataset, my_book_id, display_cover=T) {
    book_info <- books[books$book_id==my_book_id,]
...
##### 4. Our `fun_plot` function requires that the datasets `books` and `authors` have been loaded exactly right (and with the correct names) in the environment. By including and referencing objects _outside_ of our function's scope, _within_ our function (in this case the variables `books` and `authors`), our `fun_plot` function will be more prone to errors, as any changes to those objects may break our function. For example:
...
Fix this by making the datasets (`books` and `authors`) arguments in the function. Modify our function to run based on those newly added arguments.
...
count_words <- function(my_column) {
    ...
    return(length(split_col[which(split_col!="")]))
}
```
...
##### 4. Write a line of commands that creates a new dataset called `az_fl_airports.txt`. `az_fl_airports.txt` should contain a list of airport codes for all airports from both Arizona (AZ) and Florida (FL). Use the file we created in (3),`new_airports.csv` as a starting point.

##### How many airports are there? Did you expect this? Use a line of bash code to count this.

##### Create a new dataset that contains all of the data for flights into or out of Florida and Arizona (using the `2008.csv` file). Use the newly created dataset, `az_fl_airports.txt` to accomplish this.
...
```{bash, eval=F}
cat az_fl_airports.txt | wc -l # should be 484705
```
...
````markdown
```{sql, connection=lahman, eval=F}`sql ''`
SELECT * FROM batting LIMIT 1;
```
````
...
##### 3. Write a line of commands that creates a new dataset called `az_fl_airports.txt`. `as_fl_airports.txt` should  contain a list of airport codes for all airports from both Arizona (AZ) and Florida (FL). Use the file we created in (3),`new_airports.csv` as a starting point.
...
##### 4. Write a bash script that accepts the year as an argument and performs the same operations as in question 4, returning the number of flights into and out of both AZ and FL for any given year.
...
##### 5. Use the newly created dataset, `az_fl_flights.csv`, from question 4 to calculate the total number of flights into and out of both states by month, and by year, for a total of 3 columns (year, month, flights). Export this information to a new file called `snowbirds.csv`.
...
##### 9. Get a list of the following by year: wins (`W`), losses (`L`), Home Runs Hit (`HR`), homeruns allowed (`HRA`), and total home game attendance (`attendance`) for the Detroit Tigers when appearing in a World Series (`WSWin` is `Y`) or when league champion (`LgWin` is `Y`).
