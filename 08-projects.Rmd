...
##### 1. Load up the following two datasets `goodreads_books.csv` and `goodreads_book_authors.csv`into two data.frames `books`, and `authors`, respectively. How many columns and rows are in each of the three datasets?
...
##### 4. `awk` is a really good tool to quickly get some data and manipulate it a little bit. For example, let's see the number of kilometers and miles traveled altogether in 1990. To convert from miles to kilometers, simply multiply by 1.609344. 
