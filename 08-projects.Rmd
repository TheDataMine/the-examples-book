...
##### 4. Write a line of commands that creates a new dataset called `az_fl_airports.txt`. `as_fl_airports.txt` should  contain a list of airport codes for all airports from both Arizona (AZ) and Florida (FL). Use the file we created in (3),`new_airports.csv` as a starting point.
...
**Relevant topics:** [awk](#awk), [echo](#echo), [wc](#wc), [piping](#piping-and-redirection)
...
**Relevant topics:** [SELECT](#sql-examples), [FROM](#sql-examples), [WHERE](#sql-examples), [LIMIT](#sql-examples)
...
##### 5. Get the Cleveland (CLE) Pitching Roster from the 2016 season (`playerID`, `W`, `L`, `SO`). Order the pitchers by number of Strikeouts (SO) descendingly.
...
##### 6. Find the 10 team-year combinations that have the most number of Errors (`E`) between 1960 and 1970. Display their Win and Loss counts too. What is the name of the team that appears the 3rd place in the ranking of team-year combinations?
...
##### 8. Find the [AL West](https://en.wikipedia.org/wiki/American_League_West) (use `lgID` and `divID` to specify AL West) home run (`HR`), walk (`BB`), and stolen base (`SB`) totals by team between 2000 and 2010. Which team-year combination led in each category in the decade?
...
##### 9. Get a list of the following by year: wins (`W`), losses (`L`), Home Runs Hit (`HR`), homeruns allowed (`HRA`), and total home game attendance (`attendance`) for the Detroit Tigers when appearing in a World Series (`WSWin`) or when league champion (`LgWin`).
