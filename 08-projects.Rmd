...
Unless otherwise specified, all projects will need 1-3 submitted files: 

1. A compiled PDF file (built using the template), with all code and output.
2. If it is a project containing R code, a .R file containing all of the R code. _Note: This is **not** an .Rmd file._
3. If it is a project containing Python code, a .py file containing all of the Python code.
...
**Relevant topics:** [reading data in r](#r-reading-and-writing-data), [head](#head)
...
##### 2. We've provided you with R code below that will extract a column of our `data.frame` into a vector. What is the first value in the vector? What is the 50th value in the vector? What type of data is in the vector?

```{r, eval=F}
our_vec <- meta$WDWMAXTEMP
```

**Relevant topics:** [lists & vectors](#r-lists-and-vectors), [indexing in r](#r-indexing), [type](#r-type), [creating variables](#r-variables)
...
##### 3. You can access many elements in a vector at the same time. Create three new vectors named: `first50`,  `last50`, and `mix`. The vector `first50` should contain the first 50 values of the `our_vec` vector we created in the previous question, and `last50` should contain the last 50 values. The vector `mix` should contain the value resulting from each element of `first50` being added to each element of `last50`.
...
**Relevant topics:** [indexing in r](#r-indexing), [creating variables](#r-variables), [length](#r-length)
...
##### 4. In (3) we were able to rapidly add values together from two different vectors. Both vectors were the same size, hence, it was obvious which elements in each vector were added together. Create a new vector called `hot` which contains only the values from the vector `our_vec` which are greater than or equal to 80 (Note: the vector `our_vec` contains max temperatures for days at Disney World). How many elements are in `hot`? Calculate the sum of the vectors `hot` and `first50`, do we get a warning? Read [this](https://excelkingdom.blogspot.com/2018/01/what-recycling-of-vector-elements-in-r.html) and then explain what is going on. 

**Relevant topics:** [logical indexing](#r-indexing), [logical operators](#r-logical-operators), [length](#r-length), [recycling](#r-recycling)
...
**Relevant topics:** [recycling](#r-recycling)
...
**Relevant topics:** [colnames](#r-colnames-and-rownames), [which](#r-which)
...
**Relevant topics:** [cut](#cut), [dates](#r-dates), [factor](#r-factors), [paste0](#r-paste), seq, [length](#r-length), [unique](#r-unique)
...
**Relevant topics:** [for loop](#r-for-loops), [read.csv](#r-reading-and-writing-data), [pie](#pie), [table](#r-table)
...
**Relevant topics:** [functions](#r-writing-functions), [paste0/paste](#r-paste), [grep](#r-grep)
...
##### 4. Your co-worker wrote a report, and has asked you to beautify it. Knowing Rmarkdown, you agreed. Spruce up the report below. At a minimum:
...
- Make the calls to the `library` function and the `install.packages` function be evaluated but not displayed. 
...
- Spruce up the "document", and place it all under the Question 4 header in your template.
```
...
##### 6. Insert the following code chunk under the Question 6 header in your template. Try knitting the document. Something should go wrong. Fix the problem and knit again. If another problem appears, fix it. What was the first problem? What was the second problem?
...
**Relevant topics:** *[cut](#cut), [grep](#grep), [ls](#ls), [wc](#wc), [redirection](#piping-and-redirection)*
...
**Relevant topics:** *[grep](#grep), [grepl](#r-grep)*
...
##### 5. Your co-worker wrote a report, and has asked you to beautify it. Knowing Rmarkdown, you agreed. Spruce up the report below. At a minimum:
...
##### 7. Insert the following code chunks under the Question 7 header in your template. Try knitting the document. Something should go wrong. Fix the problem and knit again. If another problem appears, fix it. What was the first problem? What was the second problem?
...
**Relevant topics:** [grep](#grep), [grepl](#grep)
...
**Relevant topics:** [length](#r-length), [grep](#grep)
