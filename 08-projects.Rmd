...
##### 1. In previous projects we learned how to get a single column of data from a csv file. Write 1 line of UNIX commands to print the 15th column, the `ORIGIN`, from `1987.csv`. Write another line, this time using `awk` to do the same thing. Which one do you prefer, and why?
...
**Hint:** To actually test out your bash script, take the following example. The script is simple and just prints out the first two arguments given to it:
...
If you simply drop that text into a file called `my_script.sh`, located here: `/home/$USER/my_script.sh`. If you run the following:
...
Doing something similar using the terminal in RStudio would be an excellent way to test out your script. In this example, if we were to turn in the "content of your bash script (starting with "#!/bin/bash") in a code chunk, our solution would look like this:
