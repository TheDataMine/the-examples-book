...
##### 4. The `episode_title_id` column in the `episodes` table references titles of individual episodes of a TV series. The `show_title_id` references the titles of the show itself. With that in mind, write a query that gets a list of all of the episodes and their titles of _Grey's Anatomy_.
...
**Relevant topics:** [INNER JOIN](#sql-inner-join), [aliasing](#sql-aliasing), subqueries, [ORDER BY](#sql-order-by), DESC, LIMIT
...
##### 7. Write a query that adds the rating to the end of each episode. To do so, use the query you wrote in (6) as a subquery. ich episode has the highest rating? Is it also your favorite episode?
...
**Relevant topics:** [INNER JOIN](#sql-inner-join), [aliasing](#sql-aliasing), [GROUP BY](#sql-groupby), [AVG](#sql-avg)
...
**Relevant topics:** [MAX](#sql-max), subqueries, [GROUP BY](#sql-groupby), [INNER JOIN](#sql-inner-join), [aliasing](#sql-aliasing)
