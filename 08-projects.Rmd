...
##### 2. The file `/class/datamine/data/fars/states.csv` contains the names of states along with a respective code. Read `/class/datamine/data/fars/states.csv` into a dataframe called `state_names`. Use our new dataframe, `state_names`, to map the codes found in the `STATE` column of our `dat` dataset from question (1) from the codes to the actual names of the states. If you are stuck on how to approach this problem, use the hints below to help walk you through this.

**Hints:**

- Make sure to first remove states from `state_names` that are not in the column `STATE` of the `dat` dataset.
- Create a new variable containing the `STATE` column (from the `dat` dataset) converted to a factor.
- Reorder `state_names` by `code` using the `order` function.
- Use the `levels` function to set the levels of our new variable as the reordered state names in `state_names`.
...
##### 3. Make a state-by-state classification of the average number of drunk drivers in an accident. Which state has the highest average number of drunk drivers per accident?
...
##### 5. How many drunk drivers are involved, on average, in crashes that occur on straight roads? How many drunk drivers are involved, on average, in crashes that occur on curved roads? Solve the pair of questions in a single line of R code.
...
##### 6. Break the day into portions, as follows: midnight to 6AM, 6AM to 12 noon, 12 noon to 6PM, 6PM to midnight, other. Find the total number of fatalities that occur during each of these time intervals. Also, find the average number of fatalities per crash that occurs during each of these time intervals.
