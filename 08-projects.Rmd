...
**Hint:** To prepare for Question 3, look at the entire row of the questions data frame that has the question about google.  The first entry on this row tells you the question that you want to look for in the users data frame in Question 3.
...
**Hint:** If you look at the question column (`text` in the `questions` dataframe) for this question, you should notice that this column is a `chr` with two possible answers: "No. Why spoil the mystery?" and "Yes. Knowledge is power!". If you start by creating a function that calculates the percentage of people who answer each question, you could use `tapply` in combination with this function to break the answer down by gender.
...
##### What percentage of questions have 1, 2, 3, and 4 options? Create a new column in the `questions` dataset called `number_options` which contained the number of answer options for each question.
...
##### 4. How many times has Giancarlo Stanton struck out in years in which he played for "MIA" or "FLO"?
