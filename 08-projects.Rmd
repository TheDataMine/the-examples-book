...
- 1-2 sentences commenting on what you decided to include and why.
```
...
**Hint:** Note that if you used the `fread` function from `data.table` to read in the data, you will not need to convert the columns to the `Date` class.
...
##### 5. Using the combined `selected_projects` data, get the school(s) (`School.Name`), city/cities (`School.City`) and state(s) (`School.State`) for the teacher with the highest number of `Fully Funded` projects (`Project.Current.Status`).
...
**Hint:** To get only certain columns when subsetting, you may find the argument `select` from `subset` useful.
...
**Important note:** It is okay to get "State Not Found" for some of the addresses.
...
##### 2. How many, and what are the unique `type`s from the `titles` table? For the year found in question (1) with the most `titles`, find the number of times that each `type` appears during that year.
...
- A list of `type` and count for the year (`premiered`) that had the most `titles`.
```

##### F.R.I.E.N.D.S is a popular tv show. They have an interesting naming convention for the names of their episodes. They all begin with the text "The One ...". There are 6 primary characters in the show: Chandler, Joey, Monica, Phoebe, Rachel, and Ross. Let's use SQL and R to take a look at how many times each characters' names appear in the title of the episodes.  (This is the goal of Questions 3, 4, 5.)

##### 3. Write a query that gets the `episode_title_id`, `primary_title`, `rating`, and `votes`, of all of the episodes of Friends.

**Hint:** If you use `episodes e` and you use `titles t`, then you will have `e.show_title_id = tt0108778` and you will have `e.episode_title_id = t.title_id`). 

**Hint:** You can slightly modify the solution to question (5) in project 13.

**Hint:** This query will take a few minutes to run.
...
F.R.I.E.N.D.S is a popular tv show. They have an interesting naming convention for the names of their episodes. They all begin with the text "The One ...". There are 6 primary characters in the show: Chandler, Joey, Monica, Phoebe, Rachel, and Ross. Let's use SQL and R to take a look at how many times each characters' names appear in the title of the episodes.  (This is the goal of Questions 1, 2, 3.)

#### Questions 

##### 1. Write a query that gets the `episode_title_id`, `primary_title`, `rating`, and `votes`, of all of the episodes of Friends.
