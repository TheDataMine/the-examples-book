...
##### 4. We can use the library `revgeo` to get a location given a set of longitude and latitude values. `revgeo` uses a free API hosted by photon in order to do so. For example:

```{r, eval=F}
revgeo(longitude=-86.926153, latitude=40.427055, output='frame')
```

##### There are over 4 million rows in our dataset -- we do _not_ want to hit photon's API that many times. Instead, we are going to do the following:
...
**Relevant topics:** [functions](#r-apply-functions), sqldf
...
**Relevant topics:** [functions](#r-writing-functions), sqldf
...
##### 5. Use the `leaflet`, `addTiles`, and `addCircles` functions from the `leaflet` package to map our latitude and longitude data to a map.
