...
**Relevant topics:**  [read.csv](#r-read), [subset](#r-subset), [indexing](#r-indexing), [merge](#r-merge), [table](#r-table), [prop.table](#r-table), [which.max](#r-which)
...
##### 4. Follow the directions [here](#r-ggmap) to load `ggmap` using `library(ggmap)` (**note:** you do _not_ need to install `ggmap`, it is already installed) and get an API key. There are over 4 million rows in our dataset -- we do _not_ want to hit Google's API that many times, nor would that work. Instead, do the following:
