...
Let's configure this so every time you access https://notebook.scholar.rcac.purdue.edu/ or https://rstudio.scholar.rcac.purdue.edu/, you will have access to the proper kernel, and the default version of python is correct. Connect to Scholar, and on Scholar, use your favorite text editor to open `~/.bash_profile`, and add the following line to the end of the file:

`source /class/datamine/data/config-f2020-s2021.sh`

Note, it is possible that this file does not yet exist. If this is the case, create it.

After the file has been edited, open a terminal and run the following:
...
If you would like to use the Python environment from within a terminal on Scholar, run the following:

```{bash, eval=F}
module use /class/datamine/data/python/f2020-s2021/modules
module load conda-env/env-py3.8.3
```
