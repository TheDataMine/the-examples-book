...
.Deliverables
====
- A printed summary of the parsed `results.json` (at minimum: experiment name, number of runs, and the first run's result).
- A short explanation (2-3 sentences) of how the bind mount is being used to pass inputs in and get results out of the container.
- One small custom change to `config.json` (for example, change `num_runs` or `samples_per_run`) and notebook output showing how that change affected the results.
====
...
.Deliverables
====
* Output from the `job_error.log` file containing:
...
* A short explanation (3-5 sentences) describing:
