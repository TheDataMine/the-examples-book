...
Context: We will be working with two separate dataframes in Python, and will learn about and make comparisons through different methods of grouping.
...
The `.groupby()` method is used to split a dataframe into groups, apply a function to each group, and to comnine the results. This is used in a similar fashion to the `tapply()` function in R's `dplyr` library. The `.groupby()` function has format `df.groupby(KEY)[COLUMN].FUNCTION()`, where 
...
Take the `high_rated` column. Create a new dataframe `sports_hits` which only contains the Sports genre of `video_games` for the games that have been highly rated.
...
Create a new dataframe `sports_fails` which contains the sports video games that are listed in as `low_rated`. In this dataframe, which games have critic score of 2.8?
...
Finding the rating per review can help to find any under-reviewed apps that have high ratings but a lower vote count. 

The rating-per-review calculation is numerical, but currently the `Reviews` column is full of character data. This will not work. 
...
# This line will cause an error message to appear
...
image:ValueError.png[]

Remove the "3.0M" value from `Clean_Reviews`, and run the line with `.astype(float)` again. 

[NOTE]
====
The comparison operator "!=" means take values where it is not equal to ___.
====
...
.Deliverables
====
5.1 Show how many unique Content Rating values there are +
