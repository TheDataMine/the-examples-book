...
== Questions
image::formula_1_circuits.png[Map showing formula one race tracks, title = "Map showing formula one race tracks. Source: https://en.wikipedia.org/wiki/List_of_Formula_One_circuits[Wikipedia list of formula one circuits page], accessed: 04/06/2026.", align=center, width=70%]
...
[IMPORTANT]
====
Customizations for the markers generally go within where you are creating them. This means that when you create map markers in Python (typically using libraries like `Folium` or `Plotly`), you pass styling and behavioral options directly inside the marker creation call, rather than configuring them separately afterward.
====
...
The Branca library has a function `.LinearColormap()`. Whithin this, you can set colors to be a list of whatever color names/hexes you would like, and a gradient will be created. 
...
There are some more default color gradients you can learn about from the https://python-visualization.github.io/folium/latest/advanced_guide/colormaps.html[python visualization colormaps page].
