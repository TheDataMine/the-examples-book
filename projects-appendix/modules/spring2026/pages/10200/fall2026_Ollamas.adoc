...
Ollama is a tool that lets you run LLMs locally on your computer or a server without too many complex commands. Rather than calling an online API, Ollama downloads the chosen model and runs in directly in your environment. 

Put simply, Ollama let's you :
...
To begin working with Ollama, we first need to make a symbolic link to where the Ollama files are being stored on Anvil. 
...
In a new Terminal window on Anvil, run these lines to create an Ollama symbolic link - this only needs to be done *once* - to set up your environment: 
...
Very helpful information about symbolic links and Ollama in general on Anvil can be found in https://the-examples-book.com/tools/python/ollama_llms[this page on LLMs in The Examples Book]
==== 

To go and launch an Ollama server, we will be using a Terminal window rather than %%bash cell magic in your notebook. 
...
This should output some text about the now running server! We will leave the server running in this tab, and will use a new window for further Terminal queries. 

In your notebook (with the Terminal still running the '....ollama serve'!), run: 
...
...to show the process status of the model(s). At this point, you shouldn't actually be running a model on the server yet, but this shows that the server is up and running without issue. 
...
Since we are creating a file that will define a model that uses 2 threads:
