...
Now we can calculate the ratings per review values with both the *numeric* columns `Rating` and `Numeric_Reviews`. Divide the `Rating` column by the `Numeric_Reviews` column to create the new column `rating_per_review`. 
