...
Motivation: Python is a very popular tool for data analysis. Indexing and manipulating values in Python is a very useful skill. It is easy to call for the value counts of a column in a Pandas dataframe - and the results come in a pre-sorted order!
...
[NOTE]
====
There are a lot of columns in this dataset. Using `pd.set_option('display.max_columns', None)` can help to display all of them.
====

Take a look at the `rating` column. It would be interesting to see how evenly these different rating levels are distributed across the 16638 rows of movie data. Look at how the movie entries are sorted throughout these rating categories. 
...
Another way to become familiar with the data is by visualizing it. We will be using the `matplotlib` library. This library makes visualization in Python very easy, and often results in static, customizable, and interactive plots. 
...
[NOTE]
====
When plotting, it's important to ensure the data is easily readable and properly labeled. The individual axis points should be visable with space around them. Each plot should have a title, and both axes should be clearly named.
====
...
1.3  Plot the counts of each `rating` category. Include a title, axis labels, and at least one other customization in your plot
====
...
To start, find all of the entries that are exactly `'Comedy'`. 
...
Use the `'=='` operator to find the entries that are exactly listed as `"Comedy"`. You could look at the actual data resulting from this filtering to check that the values are indeed just "Comedy" films. Use .shape to find how many rows are of the "Comedy" genre. 
...
=== Question 4 (2 points)
Go back to the value counts of the `rating` column. 

This output shows us how many times each of the rating types corresponds to a row of the dataset. 
Let's go back to our values from the `rating` column. +
This series shows us how many times each of the rating types corresponds to a row of the dataset. 
...
Take a look at the value counts of the `tomatometer_status` column. There are just three categories here. What could we possibly find from this?

Make a table using the `.crosstab()` function from Pandas. This table should contain BOTH the (cleaned) `rating` column and the `tomatometer_status` column. Save this table as the variable `'table_data'`.
...
Create a subset of myDF. This subset will contain only the data where the `rating` column shows `'PG'` and the `tomatometer_status` is `'Certified Fresh'`.

[NOTE]
====
Try `subset_df = myDF[(myDF["rating"] == "PG") & (myDF["tomatometer_status"] == "Certified Fresh")]`.`
====
