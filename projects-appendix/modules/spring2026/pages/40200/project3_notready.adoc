...
But due to the current status of Anvil, we cannot run Docker very easily, so we are using Apptainer instead. It similar in principle to Docker but has some key differences that make it more fitted for running on HPC (High Performance Computing) environments because of those implemenation differences.
...
For simplicity we are going to manually create a `requirements.txt` to avoid issues with the `seminar` kernel environment when installing dependencies into the container. Create this in the same spot as your `hello.py` (we dont actually need pandas here, we just need something as placeholder):
...
Which should output `x86_64` again! Nice! Now we can continue with the the prebuilt SIF that now serves the same role as our "base environment" image would in an unrestricted environment.
...
Now, when changes are made to main.py or additional routes are added we only need to re-run the container using the command above, we dont need to rebuild the image which takes a lot longer.
...
To test this, in your notebook, please make a request to a route that doesnt exist and show the output:
...
- Output from making a request to a route that doesnt exist
