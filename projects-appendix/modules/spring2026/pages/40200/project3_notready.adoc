...
We are going to cover the basics of RESTful API's and Apptainer, a Docker like product. This will build a simple baseline project that is easily expandable into almost any project you would work on.

.Learning Objectives
****
- Understand what is Apptainer and how does it work
- Understand what a RESTful API is and does
...
If you have every worked on some production code then you probably ran that code in some sort of container - likely Docker. Docker is commonly used in industry.

But due to the current status of Anvil, we cannot run Docker very easily, so we are using Apptainer instead. It similar in principle to Docker but has some key differences that make it more fitted for running on HPC (High Performance Computing) environments because of those implementation differences.
...
image::containers-vs-virtual-machines.jpg[container vs virtual machine diagram, 600,450]
...
- 2-3 sentences explaining images and registry
====
...
But first lets make a simple script that we want to run inside our container. Outside of the Notebook (in terminal or file explorer), create a script called `hello.py` that logs "Hello world!" every few seconds using Python's standard library `logging` module:
...
Like just mentioned, this runs prior to `%post`. This is where we map/move the files from our local workspace to inside the container. For a small example like this we don’t need much something as simple as:
...
If you recall from last fall in TDM 301/401 Project 2 we had to do some workaround with the PATH environment variables, here is another application of where we will specify environment variables like PATH to make sure we use the correct Python and such.
...
.Deliverables
====
- In your Notebook, please run `ls [your project directory]`
- In your Notebook, please run `cat [your project directory]/hello.log`
====
...
We briefly touched on FastAPI but lets get more into it and expand on our API now that we have a baseline.
...
[NOTE]
====
Since refreshing the API will reset the database, you will need to create new items after each reload so keep that mind when trying to test the endpoints and get the correct output.
====
...
[NOTE]
====
FastAPI does automatically refresh itself every time you make a change in API definition file however since we are running it in a container you may need to rebuild the container to be able to hit the new endpoint!
====
...
-  Output from the `/dump-items` endpoint
====
...
- A frontend application that pinging the API for information
...
Apptainer is meant to be run on HPCs and is meant to run as a singular standalone executable file. Its not meant to be as flexible as Docker but is well suited for the environment it runs in and the tasks it generally is used for.
...
Lets take a look at our containerized application from a higher level. Conceptually, it can be broken into two main parts:
...
Now go to your `api/main.py` file and add the new route and feel free to implement whatever functionality in it you would like (you can just return a simple string if you want). Then you can simply rerun command above to rerun the container and in another cell run the call again and it should work!

.Deliverables
====
- Output from running `uname -m`
- Output from making a request to a route that does not exist
- Output from making a request to a new route you added
- Brief explanation (1–2 sentences) of why bind-mounting enables faster development
====
