...
That sounds nice but if you have heard of virtual machines you may ask why would we need containers? Well, there a few good reasons. 
...
The https://www.bmc.com/blogs/containers-vs-virtual-machines/[image below] provides a visualization on the differences between conatiner and virtual machine. Containers will create a basic filesystem for your program to use while running but it will not bother running an entirely new OS layer for every container deployed.

image::containers-vs-virtual-machines.jpg[conatiner vs virtual machine diagram, 600,450, loading=lazy, title="Conatiner vs virtual machine diagram (Source of the diagram: https://www.bmc.com/blogs/containers-vs-virtual-machines/ accessed at 01/31/2026)"]
...
.Deliverables
====
1.1 Two-three sentences explaining what a container is. +
1.2 Two-three setnences explaining images and registry.
====
...
Slim is a lighter-weight variant of the official Python image (smaller than the full image but still Debian-based). You often don't need the full Python installation for your app, so using slim saves image size and build time. We use slim rather than Alpine (another commonly seen one) here because many Python packages with C extensions (e.g. numpy, cryptography) build more reliably on Debian/glibc; Alpine uses musl libc and can require more work to get some wheels building. 
...
If you recall from last fall in https://the-examples-book.com/projects/fall2025/30100/project2[TDM 301 /] https://the-examples-book.com/projects/fall2025/40100/project2[TDM 401] Project 2 we had to do some workaround with the PATH environment variables, here is another application of where we will specify environment varibales like PATH to make sure we use the correct Python and such.
...
This part is optional, but we choose to add `APP_DIR` here which just gives us the option to use that as a shortcut to specify certain paths. For some more complex projects, it may be convienent to create these environment variables. One reason is that in case you refactor general structure, then you dont necessarily need to go through a bunch of different paths and fix them, you can just fix the environment variables.
...
Lets run it with (in the command line):
...
For the sake of grading, please run this version below (in the command line) - it will pipe the output into a log file we can view in our Notebook:
[source,bash]
----
...
In practice you would likely use a file logger rather than piping the stdout into a file since it is cleaner and allows you to easily standardize format the logged information. However, piping it this way provides the same functionality for our purposes here.
====

.Deliverables
====
2.1 In your Notebook, please run `ls project/containers`, +
2.2 In your Notebook, please run `cat project/containers/output.log`.
====
...
3.2 Find a real API (like GitHub, Twitter, or any public API) and identify one endpoint that uses each HTTP method (GET, POST, PUT, DELETE). Explain what one of the endpoints do and why that HTTP method makes sense for that operation. +
3.3 Output from making the API call.
====
...
We briefly introduced FastAPI earlier. Now, with a baseline in place, we will explore it in more detail and further develop our API.
...
4.2  Output from the requests sent to the API (Succesfully write, read, update, delete). +
4.3  Output from the error handling (Failed read, Failed delete). +
4.4  Output from your own POST to the endpoint. +
4.5  Output from the `/items/dump` endpoint. +
====
...
.Deliverables
====
5.1 Output from `cat api.log` showing API requests being received. +
5.2 Output from `cat client.log` showing the randomized requests made. +
...
Apptainer is meant to be run on HPCs and is meant to run as a singular standalone executable file. Not meant to be as flexible as Docker but is well suited for the environment it runs in and the tasks it generally is used for. When you are developing an app in Apptainer for an HPC then you definitely do not want to have to wait for ever for the image to rebuild entirely each time.
...
[NOTE]
====
You typically bind things like `log` directories or `file` directories if your application requires hard files locally. You do not want to bake things that change a lot and are not necessarily integral to the core functionality into the image.
...
.Deliverables
====
6.1 Output from building the base environment (`apptainer build base_env.sif BaseApptainer.def`). +
6.2 Output from running the API with bound code (showing it works with the binding method). +
6.3 Brief explanation (one-two sentences) of why this method is faster for development.
====
