...
This question has a bit of reading, but it is mostly here to explain _why_ MLOps matters. If you are already familiar with the basic ML pipeline, you can skim the first couple paragraphs, but it might still help to read it fully.
...
There are common conventions for how to organize an ML project repo, but there is no single “correct” way to do it. What we will walk through here is a basic example that is clean and modular, so you are not constantly refactoring stuff later or repeating the same setup for every new project.
...
Most of those folders are pretty self-explanatory, and we will touch on them in more detail throughout the project, primarily the `src/` directory which is usually where all your core project code lives (e.g., data loading, training functions, model definitions, etc.). We will add to it as we go.

[NOTE]
====
These are just common conventions. You do not have to follow them exactly. Use names that make sense to you as long as they are realistic and organized. **For this project though, stick with the names we use**. 
====
...
You will see it is a little bit messy! Its not awful, but its only manageable since our project is small right now. Organizing our repo now will make development _way_ easier later on.

Reminder to make sure you are in a **_clean directory_**. The only thing that should be in this directory is your Jupyter notebook and the newly cloned repository.

The current file structure should look _something_ like this:
[listing]
----
project-xx/
├── firstname_lastname_projectx.ipynb
├── intro-mlops-1/
│   ├── main.py
│   ├── results.log
│   ├── ...
...
root_path = "~/project-XX/intro-mlops-1/" # adjust as needed
...
[IMPORTANT]
====
There are a few exceptions for Python files that do not going into `src/` -- `main.py` being one of them, leave that in the root directory. There is another that we will run into later in the project, `app.py`, that should also remain in the root and not go into `src/`. 
====
...
----
# metrics.py
def calculate_accuracy(predicted, y_test_tensor):
    """Calculate accuracy"""
    return accuracy
...
def train_model(model, train_loader, X_test_tensor, y_test_tensor, criterion, optimizer, epochs=50):
...
----
# visualization.py
def plot_training_loss(train_losses, test_losses, plot_directory):
    """Plot training and test losses"""

def plot_accuracy(accuracies, plot_directory):
...
Now in `main.py` you can simply import it and create an instance of your nerual network like so:
...
# still need to have the opimiziers and criterion defined here
----

Do the same for your other logic:

- Move all data reading and preprocessing steps to `data_loader.py`.
- Move training loops, and evaluation logic to `trainer.py`.
- Move accuracy type logic to `metrics.py`.
- Move all plots to `visualization.py`.

After you modularize everything, your `main.py` should become a shorter, cleaner "driver script" following this sort of format:

[source,python]
----
from src.data_loader import load_and_preprocess_data, create_data_loaders
from src.neural_net import SimpleNN
...
if __name__ == "__main__":
    main()
----
// was kinda hoping there was a HINT one instead of NOTE for this
[NOTE]
====
...
[NOTE]
====
While `.pkl` or `.joblib` are common for scikit-learn models, `.pth` or `.pt` is the standard for PyTorch models.
====
...
So we are going to set that up for our model. https://fastapi.tiangolo.com/#example[**FastAPI**] is one of the most commonly used API libraries in Python because its fast and very simple.  Please create a Python file called `app.py` in the same level as `main.py` and copy the code below to create a `FastAPI()` object and load in your models:
...
Now we want to create a new `.post()` method with a route called `"/predict"` to can handle prediction requests. The function should intake some data as a parameter. Normally, we would do some sort of datatype validation for safety, often using the library https://docs.pydantic.dev/latest/#pydantic-examples[**Pydantic**], but we will skip that for this project. Also, the name of the function doesn't actually matter, but its good practice to make it similar to the route name.
...
To finish setting up our basic API, we just need to add this line to the bottom of `app.py`:
...
Then, from `intro-mlops-1/` in a **terminal** window, run:
...
[NOTE]
====
In the case that the port is being use and therefore you cannot start the server, try a different port (like 4041, 4042...) 
====

This opens the server up locally so that we can send a request to it from our notebook (or another terminal session etc). Now, lets make a post request to our new endpoint with some data. Run this in your **notebook**:

[source,python]
----
import requests

url = "http://0.0.0.0:4040/predict"
...
If it all worked we should see that it predicted 'virginica' as the class! 
[source,json]
----
...
[NOTE]
====
We are storing directory paths, not individual file paths. This lets us flexbily access files like `DATA_PATH / 'data.csv'` or `MODELS_DIR / 'best_model.pth'` without hardcoding the full file paths. 
====
...
Similar to how we import the functions like we did in Question 3, we can the same thing for our config variables:

[source,python]
----
from src.config import DATA_PATH, MODELS_DIR, EPOCHS, HOST_URL, PORT ...

# or (less preferred but fine)
...
----
# app.py
from src.config import HOST_URL, PORT

uvicorn.run(app, host=HOST_URL, port=PORT)
----

This makes it so we only have one source that is referenced, so if we refactor our file structure or need to change the url that we are hosting our server on, they are all in one file making it super easy to modify without missing hidden references in our codebase.
...
We just covered some basics of MLOps! This introduction glossed over some parts for simplicity sake, but there is still alot we can do and not enough space in this project. So in future projects, we may cover more libraries and ideas that allow us to make our pipelines even cleaner, such as MLFlow, PyTorch Lightning, and Pydantic!
