...
You will see it is a little bit messy. It is not awful, but it is only manageable since our project is small right now. Organizing our repo now will make development _way_ easier later on.
...
[NOTE]
====
`--reload` will automatically refresh your API server whenever changes are detected so you do not need to manually restart it!
...
Anything that can change between environments or that may be used in multiple places in your project. This also helps you create code that is easily reusable for other projects. Some of the common things you may store in your config file are:
 ...
 - Loaded environment variables (we are skipping for now, but this is a best practice you will often use in production settings for secrets like API keys or DB URIs - https://www.geeksforgeeks.org/python/how-to-create-and-use-env-files-in-python/[quick tutorial showing how to create and use .env files in Python documentation] if interested).
