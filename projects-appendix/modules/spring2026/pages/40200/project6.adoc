...
- **Attention:** This project may require additional readings beyond the assigned materials.
****
...
[NOTE]
====
Slight modifications of the function above can be used to load in and separate *all* files by whatever classes you want.
====
...
The number of samples we see for each file differs because each audio file is a slightly different length and each "sample" corresponds to a specific point in time.
...
[NOTE]
====
The second answer on https://stackoverflow.com/questions/30619740/downsampling-wav-audio-file[Downsampling wav audio file post in stack overflow] has a good explanation and graphics for down sampling (decimating):

image::downsample_example.png[Downsampling, 400, 300, loading=lazy, title="Downsampling wav audio file (Source of the diagram: https://stackoverflow.com/questions/30619740/downsampling-wav-audio-file accessed at 02/22/2026)"]

You can use `librosa` to do it like so: `y, sr = librosa.load(filename)` and then `y_8k = librosa.resample(y,sr,8000)` to resample the audio to 8000 Hz.
====
...
.Deliverables
====
1.1 Sample waveform sample rate output. +
1.2 Time domain plots. +
1.3 Two-three sentences: What patterns do you notice in the waveforms? Which emotions seem most similar/different in the time domain and why?
====
...
- Simplifies math on the signal - convolutions in time domain are now simple multiplication (more information can be seen in here in the latter half of https://www.analog.com/media/en/technical-documentation/dsp-book/dsp_book_Ch9.pdf[Chapter 9: Applications of the DFT] in 'The Scientist and Engineer's Guide to Digital Signal Processing' book of Steven W. Smith.
...
image::Fourier_transform.gif[Fourier transform, 300, 250, loading=lazy, title="Fourier transform (By Lucas V. Barbosa - Own work, Public Domain, https://commons.wikimedia.org/w/index.php?curid=24830373 accessed at 02/20/2026)"]
...
https://en.wikipedia.org/wiki/Fast_Fourier_transform[The Fast Fourier Transform] (FFT) (via `rfft`) converts a time-domain signal (`y`) into its frequency-domain representation, decomposing it into sinusoidal components -  a linear combination of sine and cosine waves that form the signal.
...
.Deliverables
====
2.1 Frequency domain plots with dominant frequencies for each emotion. +
2.2 Two-four sentences: Compare the dominant frequencies and overall spectral shapes. Which emotions have the most similar frequency distributions and why might that be? Make sure to note the magnitude scale of each emotions graph.
====
...
So far, we have looked at signals in the time domain and frequency domain separately. But what if we want to see how the _frequency content changes over time_? This is where **spectrograms** come in.
...
Lets take a look at the original segment compared to the Hann window segment:
...
The color bar on the right shows levels of decibels which correspond to the intensity of a frequency at a given point in time. This creates a nice way for us to analyze which frequencies are apparent at a given time.

.Deliverables
====
3.1 Windowing demonstration plots. +
3.2 Window comparison plots. +
3.3 Spectrogram comparison plots. +
3.4 One-two sentences: How does the Hann window improve the frequency analysis compared to no window? +
3.5 Two-three sentences: What temporal patterns do you see in the spectrograms? Which emotions show the most dynamic frequency changes over time?
====
...
.Deliverables
====
4.1 Filtered signal plots (time domain). +
4.2 Spectrogram comparison plots. +
4.3 Two-three sentences: Compare how each filter affects the signal. Which frequencies are most affected by each filter type? +
4.4 One-two sentences: Which filter would be most effective at removing background noise? Why? +
4.5 One-two sentences: Which filter preserves the most speech intelligibility? Why?
====
...
We will extract several types of features that capture different aspects of the audio signal:
...
We talked about it earlier but RMS helps to measure the loudness and overall energy of a signal. **Zero Crossing Rate (ZCR)** is defined as the number of times the signal changes sign (crosses zero) per frame. So a higher ZCR means more high-frequency/noisy/fricative content whereas a lower ZCR means more voiced/tonal content. This would typically be used as an auxiliary feature versus a main feature.
...
Mel-frequency cepstral coefficients (MFCCs) are a very common and powerful primary feature used in audio applications. These capture the spectral characteristics in a very compact representation. https://towardsdatascience.com/audio-deep-learning-made-simple-part-2-why-mel-spectrograms-perform-better-aad889a93505/[The blog post in Towards Data Science] has a some good explanations and good graphics to help explain more about MFCCs. We highly recommend taking a look at it.
...
5.2 One-two sentences: Which emotion has the highest RMS value? What does this tell us about the emotional expression? +
5.3 One-two sentences: Which emotion has the highest spectral centroid? What does this indicate about the speech characteristics? +
5.4 One-two sentences: How does the zero crossing rate differ between emotions? What does this measure and why might it vary?
====
...
[NOTE]
====
Note that just because an individual feature looks similar across these four emotions doesn't necessarily mean it lacks predictive power. Even if some Feature A looks very similar among some examples, maybe Feature A combined with Feature B andor Feature C helps differentiate.

Also note that we are looking at 4 out of the 8 emotions available in this dataset - certain features may appear more discriminative when considering the entire set of data.
====

.Deliverables
====
6.1 Graphs of `rms`, `spectral_centroid`, `spectral_rolloff`, `zero_crossing_rate` +
6.2 Graphs of `mfcc_1`, `mfcc_2`, `mfcc_3`, `mfcc_4` +
6.3 One-two sentences: Which features show the most variation between emotions? What does this suggest about their usefulness for emotion classification? +
6.4 One-two sentences: Which features show the least variation? Why might this be the case? +
6.5 One-two sentences: Based on these features, which pair of emotions would be easiest to distinguish? Which would be hardest? Why? +
====
