...
**2c. Use the code below to generate a survival timeline plot for the first 5 patients in `survival_data`, including a dashed vertical reference line at 365 days to represent a snapshot of follow-up. Write 1–2 sentences interpreting the plot and stating how many patients experienced an observed death at our point of reference versus how many were a censored outcome.**
...
for i, row in plot_df.iterrows():
    ...
    if row['status'] == 1:
        plt.scatter(row['time'], i+1, marker='x', s=100, label='Death (event observed)' if i == 0 else "")
    else:
        plt.scatter(row['time'], i+1, facecolors='none', edgecolors='black', s=100, label='Censored' if i == 0 else "")

plt.yticks(range(1, 11), [f'Patient {i}' for i in range(1, 11)])
plt.xlabel('_________') # For YOU to fill in
plt.ylabel('_________') # For YOU to fill in
plt.title('_________') # For YOU to fill in
plt.legend()
plt.show()
----
...
NumIQ. *Survival Analysis: Simply Explained*. https://www.youtube.com/watch?v=4npb5nGckYM
