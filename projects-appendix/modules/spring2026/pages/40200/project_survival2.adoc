...
* $h(t)$ is the hazard (instantaneous risk of death) at time $t$
...
* $\beta_1,\ldots,\beta_p$ are the coefficients estimated by the Cox model

Each covariate receives one coefficient.



**Example Cox Model using our dataset**

Suppose we fit a Cox model using:
...
Each coefficient measures the **log hazard ratio** of that variable while holding the others constant.



**Log Hazard Coefficients**

Example coefficients output from a fitted model:
...
| Age
| 0.02
| Positive → higher age increases risk of death

| Sex (female vs male)
| -0.60
| Negative → lower risk for females

| ph.ecog
| 0.90
| Higher ECOG score → worse health → higher risk
|===
...
Coefficients are converted into **hazard ratios** using:
...
| Age
| 1.02
| About a **2% increase in risk for each additional year**
...
| ph.ecog
| 2.46
| $HR > 1$ → more than double the risk for each 1-unit increase
|===
...
Example interpretations using our dataset:

* $HR(\text{age}) = 1.02$  
  → Each additional year of age increases the hazard of death by ~2%.

* $HR(\text{sex}) = 0.55$ (female)  
  → Female patients have about **45% lower risk** of death compared to males.

* $HR(\text{ph.ecog}) = 2.46$  
  → Patients with worse ECOG scores have substantially higher risk of death.



**Applying the Cox Model to Individual Patients**

Using rows from `survival_data`:

* Patient 0:
  - age = 74, sex = 1 (male), ph.ecog = 1  
* Patient 2:
  - age = 56, sex = 1 (male), ph.ecog = 0  

Because Patient 0 is older and has a worse ECOG score, the Cox model predicts that:

*Patient 0’s hazard of death is higher than Patient 2’s.*


**Sample Cox Output Table**

Example of Cox regression output using `survival_data`:

[cols="3,2,2,2,3"]
|===
| *Covariate* | *Coefficient ($\beta$)* | *Std. Error* | *p-value* | *Hazard Ratio (95% CI)*
...
* 95% confidence intervals:
  ...
  - CI entirely above 1 → increased risk
  - CI entirely below 1 → decreased risk
...
Since the confidence interval does not include 1 and $p < 0.05$, ECOG performance status is a **significant predictor of mortality** in this dataset.



**Key Takeaways**

* Cox coefficients represent **log hazard ratios**.
* Convert coefficients to hazard ratios using $HR = e^\beta$.
* Interpretation:

  - $HR > 1$ → increased hazard of death
  - $HR < 1$ → decreased hazard of death
