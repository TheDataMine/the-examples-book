...
In those cases, a binary modeling approach like logistic regression may seem like it can be used. However, when the **timing of the event matters**, this approach is not adequate.
...
Instead of modeling only whether an event occurs, survival analysis models:

**the time until an event occurs**.

This time is called the **survival time** or **event time**.

The survival time is measured from a clearly defined starting point until the event happens.
...
* If the event occurs before observation stops, then stem:[Y = T].  
* If observation stops before the event occurs, then stem:[Y = C].
...
Instead, it records **elapsed time in days** between study start and the outcome.  
Each recorded survival time therefore represents how many days a patient lived after diagnosis or enrollment.
...
So for each individual we have the pair stem:[(Y, \delta)].

* If stem:[\delta = 1], then the event occurred at time stem:[Y].  
* If stem:[\delta = 0], then the observation is censored and the true event time is greater than stem:[Y].
...
In other words, the effect of a predictor on risk is assumed to be **stable across the entire follow-up period**.

**Testing the Assumption**

The proportional hazards assumption can be tested statistically using tests such as the **Schoenfeld residual test**, which produces a **p-value** for each predictor.
