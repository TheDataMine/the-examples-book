...
image::mixedmodelsleepstudy.jpg[container vs virtual machine diagram, 600,450, loading=lazy, title="Container vs virtual machine diagram (Source of the diagram: https://www.bmc.com/blogs/containers-vs-virtual-machines/ accessed at 01/31/2026)"]
...
Mixed effects models are called "mixed" because they combine fixed effects and random effects within the same model.

Fixed effects describe the overall population relationship. These are the primary effects we want to estimate, similar to the coefficients in a standard linear regression.

Random effects capture group-level variation. They allow certain parameters (such as intercepts or slopes) to vary across clusters of related observations.
...
* The black line reflects the population trend  
...
**2a. Determine whether there are any zero or NA values in the `Reaction`, `Subject`, or `Days` variables. If any are present, remove those observations from the dataset.**
