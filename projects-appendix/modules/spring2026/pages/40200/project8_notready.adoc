...
= TDM 40200: Project 9 - Introduction to Mixed Models

== Project Objectives

In this project, we will introduce mixed models. 
...
- /anvil/projects/tdm/data/sleepstudy
...
Rather than forcing one line to describe everyone, a mixed model assumes:

There is an overall population trend, and individuals vary around that trend.
...
That vertical shift represents the random intercept.

In other words:

* Each subject’s line follows the same slope but starts at a different height  
...
----
import pandas as pd
sleep_study_data = pd.read_csv('~/data/sleepstudy/sleepstudy.csv')
----
...
**3b. Use the code below to overlay the regression line from Question 3a on the scatterplot of `Reaction` versus `Days`. How well does this single regression line capture the variability in the data? How do you think `Subject` might influence this relationship? Write 1–2 sentences reflecting on these questions.**
...
**4c. In 1-2 sentences explain what the major difference is between the two equations for each subject. Why do you think that is?**
...
**5c. Use the code below to xxtract and display the subject-specific random intercept effects from the mixed-effects model. What do these values represent? In 2–3 sentences, explain how these subject-level deviations modify the overall intercept and what this tells you about individual baseline reaction times.**
