...
b.  Use indexing (not the `subset` function) to make a table of the `Age` values at the time of death for which the `Race` value is the number 7 (which stands for Filipino race) and for which the `Age` is not 999.
