...
Join the `CollegePlaying` and `People` tables on the `playerID` values.  Print the `DISTINCT(playerID)` and `nameFirst` and `nameLast` values from the `playerID` table for each of the players that have `schoolID = 'purdue'` in the `CollegePlaying` table.

.Deliverables
====
- Print the `DISTINCT(playerID)` and `nameFirst` and `nameLast` values from the `playerID` table for each of the players that have `schoolID = 'purdue'` in the `CollegePlaying` table.
====
