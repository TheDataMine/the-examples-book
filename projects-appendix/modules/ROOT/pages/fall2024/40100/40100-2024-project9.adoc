...
Finally, compute the the mean squared error and r-squared for each model for each dependent variables. Then, average the results to get a single value for each metric.

[NOTE]
====
You can use the `get_mse` and `get_r_squared` functions from question 2 to calculate the evaluation metrics. However, since we have multiple dependent variables, you will need to calculate these metrics for each dependent variable and average them to get a single value for each metric. I recommend you split the y_test and y_pred arrays by column, eg. `cols = [y_pred[:, 0], y_pred[:, 1]]`, and then iterate through the columns to calculate the metrics.
====
