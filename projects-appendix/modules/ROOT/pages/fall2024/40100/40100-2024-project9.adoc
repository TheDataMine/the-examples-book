...
| Polynomial Regression | 1 variable | 1 variables | Models the relationship between one or more independent variables and one dependent variable using a polynomial function. Think y = ax^2 + bx + c, where y is the dependent variable, x is the independent variable, and a, b, and c are coefficients.
...
| Multivariate Polynomial Regression | 2+ variables | 2+ variables | Same as Multivariate Linear Regression, but for polynomials. Also generalized to Y = AX + B, however X must have all independent variables and their polynomial terms, and A and B must be much larger matricies to store these coefficients.
|===
...
After finding the best fit line, we should have two variables stored: `y`, and `y_pred`. Now that we have these, we can briefly discuss evaluation metrics for regression models. There are many many metrics that can be used to evaluate regression models. We will discuss a few of the most commone ones here, but we implore you to do further research on your own to learn about more metrics.
...
| Mean Absolute Error (MAE) | Average of the absolute differences between the predicted and actual values. | $MAE = \frac{1}{n} \sum_{i=1}^{n} |y_i - \hat{y}_i|$, where $y_i$ is the actual value and $\hat{y}_i$ is the predicted value. | $[0, \infty)$
...
Now that we understand some evaluation metrics, let's see how polynomial regression compares to simple linear regression on our same dataset. We will explore a range of polynomial degrees and see how the evaluation metrics change. Firstly, let's write a function that will take in an x value and an array of coeffecients and return the predicted y value using a polynomial function.
...
assert poly_predict(3, [1, 2, 3, 4, 5]) == 86
assert poly_predict(4, [1, 2, 3]) == 39
...
- Code to use Ridge and Lasso regression on the 5th degree polynomial
...
Given the above code for a Multivariate Linear Regression Model, can you implement a Multivariate LASSO and Ridge model? How do the coefficients of the LASSO and Ridge models compare to the Linear model?

Finally, compute the the evaluation metrics for the Multivariate Linear, LASSO, and Ridge models. Store the results in the variables `linear_metrics`, `lasso_metrics`, and `ridge_metrics` respectively.

[NOTE]
====
You can use the `get_mse`, `get_rmse`, `get_mae`, and `get_r_squared` functions from question 2 to calculate the evaluation metrics. However, since we have multiple dependent variables, you will need to calculate these metrics for each dependent variable and average them to get a single value for each metric.
====
...
- Output of the evaluation metrics for each model
====
