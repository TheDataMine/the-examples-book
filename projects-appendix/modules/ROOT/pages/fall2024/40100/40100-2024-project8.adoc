...
In this project, we will learning about Extra Trees and Random Forests, two popular ensemble models utilizing Decision Trees.
...
Random forests have 1 additional parameter compared to decision trees, `n_estimators`. This parameter simply controls the number of trees in the forest. The more trees you have, typically the more robust your model will be. However, more trees leads to longer training and prediction times, so you will need to find a balance.
