...
Firstly, if the argument 'reset_weights' is true, or if `reset_weights` is false but the weights and bias are not set, we will initialze our weights to a np array of zeros with the same length as the number of features in our input data. We will also initialize our bias to 0. In any other case, we will not modify the weights and bias.
