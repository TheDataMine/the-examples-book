...
[NOTE]
====
This concept of manually assigning the importance of features is known as feature engineering. Feature engineering is using existing knowledge of the data to either create new features or scale existing features in a way that will help the model perform better.
====

Let's create a new function in the KNN that will calculate the euclidean distance between two points, but will take a list of weights to determine how important each feature is. This will allow us to scale the distance between two points based on the importance of each feature.

[source,python]
----
def scaled_distance(self, point1, point2, weights):
    # firstly, scale weights so they sum to 1
    ''' YOUR CODE HERE '''

    # then, scale the 2 points by the weights
    ''' YOUR CODE HERE '''
...
*To ensure your implementation is correct, run the below code to calculate the scaled distance between the first row of the training set and the second row of the testing set. The printed values should be 1.4718716551800963 for euc_dist and 1.9691711992735412 for scaled_distance.*
...
Now that we have code to scale the distance between two points based on the importance of each feature, let's write a function to test the accuracy of our KNN model given weights.

[source,python]
----
def classify_weighted(self, new_point, k=1, weights=None):
    ''' If no weights are provided, run the existing classify function '''
...
*To test that your functions work, please run the below code to calculate the accuracy of the model with different weights*
...
Please use the 'euc_dist' function to calculate the distance between points, we will not use scaled features for this question.

[source,python]
----
def classify_weighted_ties(self, new_point, k=1):
...
*To test that your function works properly, we will classify the a test point at different k values. Run the below code to classify the 9th row of the testing input features using the KNN algorithm with k=5, 7, and 9. The output should be 'Iris-versicolor' for all k=5, and k=7, and 'Iris-virginica' for k=9.*

[source,python]
----
knn = KNN(X_train, y_train)
print(knn.classify_weighted_ties(X_test[8], k=5))
print(knn.classify_weighted_ties(X_test[8], k=7))
print(knn.classify_weighted_ties(X_test[8], k=9))
----
...
.Deliverables
====
- Classification of the 9th row of the testing input features using the KNN algorithm with k=5, 7, and 9
