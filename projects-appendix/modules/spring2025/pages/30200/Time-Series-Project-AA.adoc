...
This dataset comes from link:https://www.ncei.noaa.gov/pub/data/ghcn/daily/readme.txt[NOAA Local Climatological Data] and includes variables such as daily temperature, precipitation, humidity, wind speed, and snowfall for Indianapolis.

These are the key environmental indicators we’ll work with:

- `DailyAverageDryBulbTemperature`: Average air temperature (°C)
- `DailyMaximumDryBulbTemperature` and `DailyMinimumDryBulbTemperature`: Max and Min Air Temperature for the day (°C)
- `DailyPrecipitation`: Rainfall or melted snow (mm)
- `DailyAverageRelativeHumidity`: Moisture in the air (%)
- `DailyAverageWindSpeed`: Wind patterns (mph)
- `DailySnowfall`: Snow accumulation (mm)

We’ll focus on how these weather variables change over time. That means we need our data to be consistent—each day represented exactly once, variables stored as numbers, and timestamps in datetime format. This is what allows us to do time series modeling using tools like ARIMA or LSTMs.



[IMPORTANT]
====
We’ll ask a series of questions to help you explore the dataset. These are meant to guide your thinking. The **deliverables** listed under each question describe what you’ll need to submit.
====
...
- 1b. Check the number of rows and columns, and inspect the column names and data types. Output this information, then write 1–2 sentences explaining which columns seem most useful for weather analysis and whether the data types look appropriate.
...
- 2c. In 1–2 sentences, explain whether you think the dataset has one day's worth of weather data per day. Based on what you see, are there multiple weather observations per day? What might you need to do to clean the data so that there's only one row of weather data per calendar day?
...
As part of this preparation, you may notice that we’ll need to remove rows that are missing all weather measurements. These rows don’t provide any usable information—there’s nothing to visualize or analyze. However, we need to be careful not to include columns like `DATE` or `NAME` when removing rows. If we do, we might accidentally keep rows that only have a date or location but no actual weather data. Keeping those rows could result in misleading patterns, inaccurate summaries, or errors later in your analysis. These rows aren’t helpful for analysis, since they don’t contain any usable data. However, we don’t want to remove rows just because they include a date or station name—we only want to drop rows where all of the selected weather columns are missing so we will exclude `DATE` and `NAME` when removing rows.


.Deliverables
====
- 3a. Create a filtered version of your dataset that includes only the columns in `columns_to_keep`. Save this as a new DataFrame.

  You may find it helpful to define the list of columns to retain and apply it to your dataset:
...
- 3b. Write code to drop any rows where all weather columns are missing, making sure to EXCLUDE `"DATE"` and `"NAME"` from the check.

  Hint: Create a list of weather-related columns (excluding `DATE` and `NAME`) and use `.dropna()` with `how='all'` as is done below:
...
- 3c. Convert the `DATE` column in your dataframe to datetime format `YYYY-MM-DD` and preview the result to confirm it worked.

  Hint: Use `pd.to_datetime()`.

- 3d. Print the shape of your cleaned dataset and display the range of dates it covers.

  Hint: Use `.min().date()` and `.max().date()`.
...
Suppose we try to interpolate missing temperature values without setting the date as the index:
...
This throws an error! The error will probably read something like "ValueError: time-weighted interpolation only works on Series or DataFrames with a DatetimeIndex."
...
To fix this you can use `.replace` and `.astype()` to convert the trace values to a numeric value like this:
...
.Deliverables
====
- 4a. Convert the DATE column to datetime and set it as the index. Identify which columns are numeric by using `.dtype()`.
...
- 4c. After interpolation, reset the index so DATE becomes a regular column again.
- 4d. Find and replace any non-numeric "T" values in weather columns (e.g., precipitation, snowfall), convert the columns to float, and confirm no non-numeric values remain.
====

=== Question 5 Exploring Climate Trends Over Time (2 points)

You’ve cleaned and prepared your data—now it’s time to visualize it. Try visualizing the full range of daily temperatures to uncover trends or shifts over the years. Then, focus on a single year. What patterns do you notice when you zoom in?

.Deliverables
====
- 5a. Review your temperature columns. They are currently in Celcius. Convert them to Farenheit (°F).
- 5b. Create a time series plot of daily average temperature (°F) from 2006 to 2024. What trends do you see?
- 5c. Create a second plot focusing only on 2024 (°F). What seasonal patterns or anomalies stand out?
- 5d. Save your new dataframe as a CSV and call it `all_years_df_indy_climate_clean`. 
====
