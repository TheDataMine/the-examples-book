...
We will start by loading and cleaning the data from the Centers for Medicare and Medicaid Services (CMS), exploring key features, and examining class balance. From there, we will dig into modeling and interpretation to better understand what kind of prescribers are behind this high profile drug.
...
The model with the lowest AIC fits the data by striking a balance between fit and # of parameters (features) used. If we pick the model with the smallest AIC we are picking the model with a low k or lowest # of parameters (features) but still making sure the model has a high likelihood log(L).
