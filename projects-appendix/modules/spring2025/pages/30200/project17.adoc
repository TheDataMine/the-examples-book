...
Each of the steps in this question prepares your data for modeling. Some of the code has been provided — your task is to fill in the missing pieces and reflect on why each step is needed.
...
_Note:_ The function `forward_selection()` below has already been written for you. It adds one feature at a time, selecting the one that most reduces the AIC, and stops when no further significant improvement is made. Your task is to run the function and then write the 2–3 sentences.
