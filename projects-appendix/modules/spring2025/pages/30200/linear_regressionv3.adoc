...
- Understand the assumptions of linear regression and what each assumption is testing for. 
...
- Simple — uses a single predictor variable (later, we will use multiple predictors).
- Math behind the model is transparent compared to more advanced black-box models. 
- Regression — we are modeling the relationship between a numeric response and one (or more) numeric predictors. In SLR, both the response and the predictor are numeric.
...
Source: https://the-examples-book.com/projects/data-sets/Spotify[Spotify Dataset 1986–2023]
...
plt.xlabel("Duration (minutes)") # For YOU to fill in 
plt.ylabel("Popularity") ) # For YOU to fill in 
plt.title("Popularity vs. Duration (train set)") ) # For YOU to fill in 
plt.show()
----

*Predictor (X)*: `loudness`  

*Response (Y)*: `popularity`

image::Regression_Line_Spotify.png[width=600, height=450, caption="Simple Linear Regression Plot Spotify Data"]
...
So, our goal is to find a line that **captures the main trend** without falling into either extreme (underfitting or overfitting).  The regression line should summarize the relationship between **popularity** (Y) and **loudness** (X) well.  

**How Do We Define a Good Line?**

We would like to use a linear function of `X`, writing our model with `β₁` as the slope:
...
\[
\text{Residual} = \text{Observed Popularity} - \text{Predicted Popularity}
\]

 

Residuals are minimized in the least squares approach.

**Least Squares Approach**

We estimate coefficients \(\hat{\beta}_0\) and \(\hat{\beta}_1\) by minimizing:
...
- Tells us how much popularity changes (on average) for each additional minute of track duration.  
...
- Linear. The relationship between stem:[Y] and stem:[x] is linear, of the form
stem:[Y = \beta_0 + \beta_1 x + \epsilon].

- Independent. The errors stem:[\epsilon_i] are independent.

- Normal. The errors stem:[\epsilon_i] are normally distributed (the “error” around the line follows a normal distribution).

- Equal Variance. At each value of stem:[x], the variance of stem:[Y] is the same, stem:[\sigma^2].
...
- **R²** tells us how much of the variation in popularity is explained by duration.  
...
== Takeaways
* Linear regression provides a transparent way to model Spotify popularity.  

* Duration alone may not explain popularity well, but it serves as a simple example.  
...
Sometimes one feature isn’t enough. Multiple Linear Regression (MLR) uses **several predictors at once**.
...
- Each coefficient \( \beta_j \) is the **expected change in \(Y\)** for a 1-unit increase in \(X_j\), **holding all the other predictors constant**.


*Why use it?*  

- More predictors can help captures more of what drives popularity (e.g., duration might not be enough for accurate predictions, but combined with more variables it can help).
...
**Cook’s Distance (Dealing with (Influential points)**

Cook’s Distance measures how much a single observation *influences the entire fitted model* (all coefficients at once).

It combines two ideas:

* **Residual size:** how far the point’s observed value is from the predicted value.

* **Leverage:** how unusual the point’s predictor values are (far from the center of `X`).

Intuition: a point with a **large residual** *and* **high leverage** can pull the regression line toward itself, changing the fit for many other points. Cook’s Distance flags such **influential** observations.



How we use it:

. Fit the regression on the cleaned training data.

. Compute Cook’s Distance \(D_i\) for each row.

. Flag potential outliers using the common rule-of-thumb **threshold** \(D_i > \frac{4}{n}\) (with \(n\) = number of training rows).

. Remove the flagged indices from **both** `X_train` and `y_train` so the two stay aligned.
...
_Note: The function operates iteratively by removing the variable with the highest VIF at each step until all remaining variables have VIF values less than or equal to the chosen threshold (commonly set at 10). Your task is to fill in the appropriate threshold._
...
**4b. Use the provided code to run the stepwise_aic function on your training data. Then print the `selected_cols` and interpret the results in 1-2 sentences.**
...
- Linear. The relationship between Y and the predictors are linear
...
image::Normality_assumption_histogram.png[width=600, height=450, caption="Normality Test Histogram Spotify Data"]
...
image::Normality_assumption_qq.jpg[width=600, height=450, caption="Normality Test Q-Q Plot Spotify Data"]
...
- At any fitted value, the average residual should be about 0. This supports the linearity assumption. Add a horizontal reference line at stem:[y = 0] to make this clear.

- Even spread (constant variance).

- Across all fitted values, the spread of residuals should be roughly the same. 

image::fitted_vs_res_spotify.png[width=600, height=450, caption="Fitted vs Residuals Plot Spotify Data"]
...
**6b. Run the code below to generate a table displaying each variable along with its coefficient, direction of effect (positive or negative), p-value, and significance. Then, write 2–3 sentences interpreting the results. Highlight any findings that stand out or seem surprising.**
