...
- Regression — we can model the relationship between a numeric response and one (or more) predictors. 
...
Cook’s Distance measures how much a single observation *influences the fitted model*.

It combines two ideas:

* **Residual size:** how far the point’s observed value is from the predicted value.

* **Leverage:** how unusual the point’s predictor values are (from the mean of `X`).

For example, a point with a **large residual** *and* **high leverage** can pull the regression line toward itself, changing the fit for many other points. Cook’s Distance flags such **influential** observations.
