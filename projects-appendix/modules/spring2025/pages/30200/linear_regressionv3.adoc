...
- Simple — can use a single predictor variable (later, we will use multiple predictors).
- Math behind the model is transparent compared to more advanced black-box models. 
- Regression — we are modeling the relationship between a numeric response and one (or more) predictors. 
...
- **Linear.** The relationship between $Y$ and $x$ is linear, of the form  
  $Y = \beta_0 + \beta_1 x + \epsilon$.
