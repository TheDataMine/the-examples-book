...
.Deliverables
====
- 3a. Use the code below to create a filtered version of your dataset that includes only the columns in `columns_to_keep`. Save this as a new DataFrame.
...
- 3b. Use the code below to drop any rows where all weather columns are missing, making sure to EXCLUDE `"DATE"` and `"NAME"` from the check. Then print the first 5 rows of the new DF. 
