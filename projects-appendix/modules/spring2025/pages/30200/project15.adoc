...
.Deliverables
====
- 4a. Ensure the DATE column is in datetime format and then set it as the index. Then identify which columns are numeric by using `.dtype()`.
...
- 4c. After interpolation, reset the index so DATE becomes a regular column again. Hint: you can use `reset_index(inplace=True)`.
