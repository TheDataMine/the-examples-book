...
// dead-link: https://vault.si.com/vault/1985/05/13/the-inside-story-about-baseball-in-1943-was-less-bounce-to-the-ounce
See https://en.wikipedia.org/wiki/Balata_ball[this Wikipedia article on the balata ball] for more reading. 
...
2.3. What is the correlation between these two variables? 

*Remember:* Correlation measures the strength and direction of the relationship between two variables—in this case, how home runs (HR) and runs batted in (RBI) move together. If the correlation is strong and positive, it means that players who hit more home runs also tend to drive in more runs.
====
...
=== Question 6 (2 points)
To show player-based differences, plot the regression lines estimated from the mixed model for 20 randomly selected players along with the main regression line.

In the model above, you used a varying intercept, where each player had an adjustment to the overall intercept.  The plot for 20 randomly selected players and the main regression trend is as follows:

image::Plot_WithRandomlySelected20.png[]

The plot added for your reference, your plot does not have to look like this. It is just an example.

.Deliverables
====
6.1. To show player-based differences, plot the regression lines estimated from the mixed model for 20 randomly selected players along with the main regression line.
====
