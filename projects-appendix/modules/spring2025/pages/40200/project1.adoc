...
This can all be done using the `cv2.cvtColor()` function, which takes in an image array and a conversion flag as arguments, and returns the image array after performing the conversion. The conversion flags are defined in `cv2`, and can be found https://pypi.org/project/opencv-python/[opencv page]. Please note that there are many many many different color spaces and conversions available, and it is not necessary to memorize them all. However, it is important to understand that these conversions exist, and that OpenCV provides a simple way to perform them.
