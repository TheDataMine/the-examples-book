...
Also near the end of the airports examples, we learned how to use the `subset` function, to find rows of data that meet a certain condition.  Use the `subset` command to display the content in the rows of the Star Wars data set that correspond to droids.  (Two hints:  Please check whether Droid(s) should be capitalized or not, and whether it is singular or plural.  You need to check the data.  There are four droids in the data.)
...
[TIP]
====
It is OK to just focus on US states, although if you checked for beers from other states, you will find that the top states are all in the USA.  If you did not limit your table to US states, that is OK.  Nonetheless, the top states are from the US.
====
