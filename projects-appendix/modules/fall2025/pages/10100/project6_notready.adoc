...
Motivation: Sometimes there is a task that needs to go through each entry of a set of data. Loops make this fairly easy and painless. 

Context: We will learn how to use conditionals and begin working with loops on actual datasets. 
...
Social media is now a huge part of everyday life. https://the-examples-book.com/projects/data-sets/SocialMedia[This dataset] aims to show the results of a cross-country survey on the usage patterns, academic impact, and relationship social media has on students (ages 18 - 24) and their lives. 
...
Write an `if - else if - else` conditional chain that classifies the lightsaber colors into:
...
Using the same color classifying conditions, build an chain of `ifelse` statements to determine the status of the wield of the lightsaber. For `color`, use the vector `colors`:
