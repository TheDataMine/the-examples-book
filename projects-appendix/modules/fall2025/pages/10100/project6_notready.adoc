...
An `if` statement runs a block of code only if a certain condition is TRUE. Something like:
...
You may notice that we saved this `ifelse` chain as a variable (`roles`). Please print the results for `colors`. 

Now, it is time to think more efficiently. We can write the same by using `switch` function
