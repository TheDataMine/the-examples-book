...
=== Question 3 (2 points)
We will work on 'while` in this question. To use for loops, you must know, or be able to easily calculate, the number of times the loop should repeat. In situations where you do not know how many times the desired operations need to be run, you can turn to the while loop. A while loop runs and repeats while a specified condition returns `TRUE`, and takes the following general form:
