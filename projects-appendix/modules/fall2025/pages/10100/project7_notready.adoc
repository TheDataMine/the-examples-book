...
Now, lets assume that you may want to do those steps for other data sets or for other columns. In this case you may need a generic function taking the data and the column and convert them. So, Build a function `convert_duration` that takes a dataframe as input and converts the `duration_ms` column into two new columns:
