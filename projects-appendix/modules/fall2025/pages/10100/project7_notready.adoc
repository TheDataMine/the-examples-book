...
Read in the Taylor Swift dataset as `ts_songs` from `/anvil/projects/tdm/data/spotify/taylor_swift_discography_updated.csv` and view the `head()` and dimensions of the data. Make sure to use `read.csv2` here, because the Taylor Swift dataset is ';' (semi-colon) delimited (rather than comma). 
