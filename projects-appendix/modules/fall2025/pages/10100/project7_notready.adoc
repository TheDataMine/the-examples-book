...
[WARNING]
====
There is a column (`track_lyrics`) that shows every lyric for each song in the dataset. Use `ts_songs \<- ts_songs[ , !(names(ts_songs) %in% "track_lyrics")]` to not include that column. You may want to use `head()` command after this cleaning to see the data.
====
