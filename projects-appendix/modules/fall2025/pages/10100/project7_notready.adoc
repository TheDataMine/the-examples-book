...
After running it, you may have noticed that values containing more than one `.` cannot be converted to numeric. This is because R expects numeric values to have at most one decimal point, so any extra `.` makes the value invalid for numeric conversion, resulting in `NA`.
