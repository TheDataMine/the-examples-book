...
= TDM 40100: Project 3 - Python Web Scraping and Images
...
cv2:
`B, G, R = cv2.split(img)` splits the original BGR image into blue, green, and red channels. The BGR image is an numpy array - (height, width, channels). height is the number of rows of pixels, width is the number of columns of pixels, and channel can be 0,1,2 for blue, green, and red respectively. 
...
.Deliverables
====
- 5a. Code and output of an image after splitting into the three RGB color channels using cv2
- 5b. Code and output of an image after splitting into the three RGB color channels using skimage
====

=== Question 6 (2 points)

YCbCr is another example of a color space. The color information is separated and described by: 

Y is "Luminance" and represents the brightness and the pixel's grayscale information.

Cb and Cr are known as "chrominance" which is the pixel's color information. 

Cb: Blue difference

Cr: Red difference

They are crucial in video and image compression; it allows effective compression while maintaining image quality that also works well with the human eyes' sensitivity.

We can get YCbCr with the code below (using cv2)
...
.Deliverables
====
- 6a. Split the image into YCbCr components and output the image for all. Include documentation.
- 6b. What does YCbCr achieve and how does it change the image? What are some applications of YCbCr? 
====
