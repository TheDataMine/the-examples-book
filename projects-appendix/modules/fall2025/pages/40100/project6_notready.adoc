...
Hint: Use `.valuecounts()` to print the count of 1s and 0s in the column `Semaglutide_drug`. Read more about `.valuecounts()` https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.value_counts.html[here].
...
Hint: Use `.valuecounts()`. Read more about `.valuecounts()` https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.value_counts.html[here].
...
**2b. Use the code below to group by `Semaglutide_drug` and calculate the mean and standard deviation of the numeric_cols. Then write 1–2 sentences on how the averages differ between the two classes and what that might suggest.**
...
plt.title("________") # For YOU to do
plt.tight_layout()
plt.show()
----
...
**4d. Using the code provided, convert all boolean (`True`/`False`) columns in your training, validation, and test sets to integers (`1`/`0`). Then write 1-2 sentenced on why true/false columsn should be converted to integers**
...
_Note:_ Your task is to run the code succesfully and write 1-2 intepreting the results. 
