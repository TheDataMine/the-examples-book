...
= TDM 30200: Project 02 - Basic Python, Environment & Dependency Management, Documentation
...
- Learn different dependency management methods. Know when they are used and problems if they are not used appropriately. Understand how to create and use them in projects.
- Creating visualizations and writing functions in Python.
- Python documentation.
****

== Dataset
The dataset is at ` ` from https://github.com/owid/co2-data. 
...
Dependency means there are external libraries and packages that are used for project features to work. 

There are several reasons why we need to manage dependencies. 
...
you will make it easier for yourself if you can recreate the setting for all code to work, and for others if there are multiple peopel working on it. When running or 

Say no dependency management has been done. The common issue that arises from this is version conflicts: two or more libraries used requires different versions than the one you installed because all of them are globally installed locally. 
Thus, we need to use virtual environments, to create a separate space for all of one project's dependencies so that regardless of what we have in our system's global installation or what other projects use, the one we are working on runs smoothly. 


We will try creating a virtual environment using the Python 3 venv module. Run below code to create one in the directory you are working in. 
...
Above creates a new environment named env_proj. You can name it anything, but it should be easily recognizable and relevant. 
...
This is because the pandas library could not be found in our current environment - Python can not import what doesn't exist. You should have noticed that we did not install anything yet. 

Let's run some installs:
[source,python]
----
...
[NOTE]
====
venv provides a very easy way to manage dependencies; however, if there are packages you need that is not done through pip install or some non Python libraries, venv won't do it for you and you might need to find another tool.
====
...
Let's install the same as we did previously:
...
From the py module, pdoc scans our python functions and classes and extract the docstring documentation. HTML page is created from that and 

To use pdoc, we need to first run the following command:
[source,python]
----
pip install pdoc
----
...
This is a basic example. pdoc should output something like below for you:
