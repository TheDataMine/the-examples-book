...
If you have already worked on the https://the-examples-book.com/projects/fall2025/20100/project8[previous week's project (project 8)], you can skip this section and begin with Question 1. If you do not have a local copy of the Lahman database from last week, please follow the steps below.

Copy the Lahman database and save it in your local folder in anvil. We can do this buy first loading the database from the path above like normal, and then using the python sqlite3 library to create a new database file in your local folder and copy the contents of the Lahman database into it.
...
[NOTE]
====
For the **Chinook** dataset, the equivalent is creating a trigger to log the creation of a new playlist into the `PlaylistActivity` table:
...
[NOTE]
====
For the **Chinook** dataset, query the `"PopularPlaylists"` view:
...
Now, query the "TallHeavyPlayers" view to get the top 5 heaviest players who were born in 1994 and are over 6 feet tall:
...
[NOTE]
====
For the **Chinook** dataset, query the `PlaylistStats` view:
...
[NOTE]
====
Instead of just using `INSERT INTO`, use `INSERT OR IGNORE INTO` to prevent the entire cell from failing if one of the statements fails. If a statement fails, it will instead just ignore that statement and continue with the rest of the statements in the cell.
====
