...
Now that you have this table, we want a trigger so that every time a player is added to the "DeathRecords" table, the same player's `deathYear`, `deathMonth`, and `deathDay` columns in the "People" table are updated to match the values in the "DeathRecords" table.
...
[source,sql]
----
DROP TABLE IF EXISTS PlaylistActivity;
