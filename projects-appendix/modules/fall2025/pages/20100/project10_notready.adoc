...
[NOTE]
====
In `df.to_dict('records')`, the `'records'` option converts each row of the DataFrame into a dictionary and returns a list of these dictionaries. This format is convenient for MongoDB because each dictionary can be inserted as a separate document. For exmple:
