...
2.4. Explain the difference between querying arrays with and without `$elemMatch`.
====
...
[IMPORTANT]
====
You need to include the `"cursor": {}` param for aggregration commands. See more https://www.mongodb.com/docs/manual/reference/command/aggregate/#mongodb-dbcommand-dbcmd.aggregate[here].
====

What you may notice here is that we have `N/A` for our execution time. If you print out `agg_explain` and look at the returned JSON, you will notice that it is structured a little different than the previous JSON return values. Aggregation commands often returns 'per-stage' stats with estimates meaning the JSON returned may be a little more dynamic than what we have previously been dealing with. 

We can handle this by 'scanning' for the execution time amongst the stages:
...
Now we see a real number estimate for execution time! (It is small amount of data so a small execution time is expect).
