...
(You may notice that, IDs are not uniform, we need one extra step in the code chunk below to make them uniform.)
...
[NOTE]
====
For this question, we have provided most of the code for you to review and follow. In the code chunk below, you only need to complete the two sections marked `YOUR CODE HERE`.
...
END{
    for (c in count)
    # Print Vendor, Average Order, Number of Orders (comma separated)
