...
Now that you have a basic understanding of the builder pattern, let's use it to create a SparkSession object.

Each SparkSession object needs the following properties:

- appName: A name for the session
- config: Configuration options for the session

Additionally, instead of ending the builder pattern with a `build()` method like in the above example, PySpark uses `getOrCreate()` to create the session if it does not already exist, or return the existing session if it does.

Putting that all together, we can create a SparkSession object like this:
[source,python]
----
import pyspark
...
.Deliverables
====
- Code to create a SparkSession object and load the weather dataset into a DataFrame
- Code to show the first 5 rows of the DataFrame
====
...
- Code to perform the desired set of operations on the dataframe and time the results
- Code to print the first 5 rows of the result DataFrame and time the results
====
...
.Deliverables
====
- Code to create a temporary view of the DataFrame
- Code to write a SQL query to perform the desired set of operations on the dataframe and time the results
- Code to print the first 5 rows of the result DataFrame and time the results
- How long did it take to print the first 5 rows of the result DataFrame? 
- How does the time it took to print the first 5 rows of the result using PySpark SQL compare to the time it took to print the first 5 rows of the result using the traditional PySpark DataFrame API?
====
...
.Deliverables
====
- Code to convert the `observation_time` column to a timestamp type
- Code to create a new column that stores the year of the timestamp
- Code to create a new column that stores the month of the timestamp
- Code to group the data by the month, and aggregate the temperature and solar_radiation by the average
- Code to sort the results by the average temperature in descending order
- Code to print the top 5 results
====
...
.Deliverables
====
- Code to use the `explain` method with `mode="cost"` to print out the Physical Plan and the Optimized Logical Plan for your code from Question 4
- How many steps did your Physical Plan have, and how many did the Optimized Logical Plan have?
- Create a new SQL or PySpark DataFrame API query that is as complex as you can, but don't actually execute it.
- Use the `explain` method with `mode="cost"` to print out the Physical Plan and the Optimized Logical Plan for your new complex query.
- How many steps did your Physical Plan have, and how many did the Optimized Logical Plan have?
- Was the Optimized Plan easier to understand than the Physical Plan?
====
