...
Firstly, we should be able to fully recover all the `book_cover` images from our database alone. This means we'll need to handle scraping the image from the `image_url` in our JSON file and converting the image to `bytes` before inserting into the database. Take a look at xref:fall2022:30100/project4.adoc#_question_2[this question] from TDM 30100, and write a function that, given an image url, returns the image as a bytes object.
