...
.. Display the first 20 rows output to the screen

[NOTE]
====
- Streaming query for products with more than 10 views can be very complex due to the need for stateful operations. In this project we will only need to use simplified conceptual approach like
[source,python]
----
...
[source,python]
----
filtered_reviewDF = myDF.join(filtered_reviewCounts,"ProductId")
----
====
