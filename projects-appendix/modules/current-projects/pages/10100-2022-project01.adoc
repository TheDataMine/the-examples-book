...
For this course, projects will be solved using the Anvil computing cluster: https://www.rcac.purdue.edu/compute/anvil[Anvil].
...
In question (1) we answered questions about cores and memory for the Brown clusters. To do so, we needed to perform some arithmetic. Instead of using a calculator (or paper, or mental math for you good-at-mental-math folks), write these calculations using R _and_ Python, in separate code cells.
