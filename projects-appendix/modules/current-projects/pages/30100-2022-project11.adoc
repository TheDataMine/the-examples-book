...
+
. Click enter twice to _not_ enter a passphrase (for convenience, if you want to follow the other instructions, feel free).
. We need to make sure the permissions are correct for your `.ssh` directory and the files therein, otherwise `ssh` will not work properly. Run the following commands from a command prompt (cmd shell):
+
[source,bash]
----
# from inside a command prompt (cmd shell)
