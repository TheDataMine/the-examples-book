...
In the previous project, we used a tool called `awk` to parse through a dataset. This was an example of running bash code using the `f2021-s2022` kernel. Aside from use the `%%bash` magic from the previous project, there are 2 more straightforward ways to run bash code from within Jupyter Lab.
...
**Relevant topics:** https://the-examples-book.com/data-science/unix/pwd[pwd]
...
**Relevant topics:** xref:data-science:unix:pwd.adoc[pwd], xref:data-science:unix:cd.adoc[cd], xref:data-science:unix:echo.adoc[echo], xref:data-science:unix:ls.adoc[ls]
...
. Confirm you are in the correct directory using the `echo` command.
...
. Confirm you are in the corrrect directory using the `echo` command.
...
**Relevant topics:** xref:data-science:unix:pwd.adoc[pwd], xref:data-science:unix:cd.adoc[cd], xref:data-science:unix:special-symbols.adoc[. & .. & ~]
...
. Execute the `/anvil/projects/tdm/bin/tokei` command, with input `~x-kamstut/bin`.
+
[NOTE]
====
Doug Crabill is a the compute wizard for the Statistics department here at Purdue. `~dgc/bin` is a directory (on a different cluster) he has made publicly available with a variety of useful scripts. I've copied over those files to `~x-kamstut/bin`.
====
+
...
**Relevant topics:** xref:data-science:unix:pwd.adoc[pwd], xref:data-science:unix:cd.adoc[cd], xref:data-science:unix:head.adoc[head], xref:data-science:unix:tail.adoc[tail], xref:data-science:unix:wc.adoc[wc], xref:data-science:unix:du.adoc[du], xref:data-science:unix:which.adoc[which], xref:data-science:unix:type.adoc[type]
...
**Relevant topics:** xref:data-science:unix:cp.adoc[cp], xref:data-science:unix:rm.adoc[rm], xref:data-science:unix:touch.adoc[touch], xref:data-science:unix:cd.adoc[cd]
