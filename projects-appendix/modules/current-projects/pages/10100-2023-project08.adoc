...
**Motivation:** We are going to take a step back and work towards building a beer reccomendation system. As you know already that a key part of writing effecient code is being able to write functions. 
...
----
====
We want to load *only* two `pandas` data frames to use in this project. We will not be working with the other dataframes.
[source, python]
----
...
Typically we can assume that we surround ourselves with people with similar interests and we would have aligning tastes in resturants and buisnesses. 
Lets check that by writing a function called `get_friends_data` with the `user_id` as an argument and returns a dataframe that contains the user's information. 

Cavat is that in this function we want to add `type hints`. 
Type hinting is a formal way to indicate the type of value in your python code. 
You can learn more about python type hints https://www.pythontutorial.net/python-basics/python-type-hints/[here] 
