...
.. Create a new data frame with the name `newDF` in which the *"SPOSTMIN"* colummn has all `NA` values removed.  In other words, select the rows of `myDF` for which *"SPOSTMIN"* is not `NA` and call the resulting `data.frame` by the name `newDF`.
...
.. Make a boxplot and a dotchart that illustrate the data for the average wait times.   Which one conveys the infomation better and why?
