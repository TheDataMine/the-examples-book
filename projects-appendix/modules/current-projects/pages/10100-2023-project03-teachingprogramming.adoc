...
To answer question~2, we want you to work with the `year` column, and try the same things we tried above.  We were simply giving you examples using the `lat` column, so that you have an example about how to deal with missing data.
...
==== Question 5 (2 pts)
.. In question 3, we found the average price of vehicles by year. Choose at least two different plot types in R, and create two plots that show the average vehicle price by year.
