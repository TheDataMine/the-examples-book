...
Secondly, take a look at the `head` of both dataframes. You will notice, the `station_id` in the `obs` dataframe appears to correlate with the `id` column in the `stations` dataframe. This is a fairly common occurrence when data has been _normalized_ for a database. 
