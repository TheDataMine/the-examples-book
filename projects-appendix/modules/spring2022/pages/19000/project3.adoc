...
// dead-link: xref:spring2022/19000/19000-s2022-project02.adoc#question-4[question (4) from the previous project]
...
Great! Now you have your `volume_dict`. Write a loop that loops through your `volume_dict` and prints the `Store Number` and `Volume Sold (Gallons)` for each key. If the volume sold is less than 100000 use the `continue` keyword to skip printing anything. If the volume sold is greater than 149999, print "HIGH: " before the store number, if the volume sold is less than 150000 print "LOW: " before the store number. 
