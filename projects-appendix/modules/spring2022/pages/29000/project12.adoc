...
[TIP]
====
The `glimpse` function is a good function to get a sample of many columns of data at once. Although it may be more difficult to read at first (than say, `head`), since it lists a single row for each column, it is better when you have many columns in a dataset.
...
Begin this question by loading the `lubridate` package. Use the appropriate function to convert the columns referring to dates to a date format. 
