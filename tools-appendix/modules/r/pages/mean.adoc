...
[NOTE]
====
// dead-link: https://www.statmethods.net/input/missingdata.html
As mentioned https://www.statmethods.net/data-input/missingdata.html[here], `NA` indicates a missing datum/value, while `NaN` indicates an impossible number (division by zero). We'll show in Example 2 that `na.rm` does not care about the difference.
====
