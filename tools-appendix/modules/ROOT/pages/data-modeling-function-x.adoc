...
// dead-link: xref:data-modeling/choosing-model/predict-infer.adoc[the Prediction vs. Inference Starter Guide page.]
In general, there are two things that we want from data: to *predict* and/or to *infer*. You can learn more about prediction and inference at the Prediction vs. Inference Starter Guide page. For our purposes, we are going to look at what estimating $f$ looks like in a prediction context.
...
// dead-link: xref:data-modeling/resampling-methods/cross-validation/train-valid-test.adoc[training data]
