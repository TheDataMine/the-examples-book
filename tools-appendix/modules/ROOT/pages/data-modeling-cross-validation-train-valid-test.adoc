...
Many machine learning packages do not have validation splits built into either the model itself (such as within unsupervised learning models: if we don't have a way to evaluate performance, then what use is a validation set?) or in the toolkits themselves https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.train_test_split.html[cross validation package only includes train and test] (https://datascience.stackexchange.com/questions/15135/train-test-validation-set-splitting-in-sklearn[although there are some simple workarounds to make validation sets with it nonetheless]). 
...
// dead-link: xref:data-modeling/resampling-methods/cross-validation/loocv-kfold.adoc[LOOCV or k-fold CV]
...
// dead-link: xref:data-modeling/choosing-model/supervision.adoc[Unsupervised learning]
