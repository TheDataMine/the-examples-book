...
// dead-link: xref:templates.adoc[here]
// dead-link: xref:submissions.adoc[here]
...
For this first question, we're going to have you get used to working with Jupyter. To start, upload https://the-examples-book.com/projects/_attachments/project_template.ipynb[this project template] to Jupyter and fill in your name and the project number as needed, selecting `seminar` as your xref:unix:unix-kernels.adoc[kernel].

Then, run the following Python in a code cell. For a more in-depth reminder on working in Jupyter, feel free to take a look at xref:projects:fall2024:10100/project1.adoc[this year's 10100 project 1] or xref:anvil:jupyter.adoc[this guide on Jupyter].
