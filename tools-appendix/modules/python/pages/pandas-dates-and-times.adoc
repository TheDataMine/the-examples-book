...
The `Timedelta` and `DateOffset` objects are simple: they are time/date objects https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#overview[different from date times] that can be supplemented a difference between dates (4 days from now, 4 months ago, etc) to find the desired future/past date. The key difference is that `Timedelta` works with absolute time (1 day = 24 hours), while `DateOffset` works with calendar time, accounting for things such as daylight savings time. `DateOffset` also lacks an array class corresponding to `timedelta64[ns]`.
