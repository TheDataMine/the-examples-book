...
`int` values can be added, subtracted, or multiplied without changing the variable type. *Unlike other languages*, however, division of `int` values will change the variable type to <<float, float>>, _meaning truncation does not happen_:
