...
[source,python]
----
diabetes_prescriber_data['Cost_per_claim'] = (diabetes_prescriber_data['Tot_Drug_Cst'] / diabetes_prescriber_data['Tot_Clms']) # For students to fill in

three_cols = ['Tot_Drug_Cst','Tot_Clms','Cost_per_claim']

three_cols_data = diabetes_prescriber_data[three_cols]

three_cols_data.head()
----
...
print(diabetes_prescriber_data["Prscrbr_State_Region"].unique()) 
...
[source,python]
----
print(diabetes_prescriber_data["Prscrbr_State_Region"].value_counts(dropna=False)) 
----
...
image::cormatrixpres.png[width=600, height=450, caption="Figure 1: Correlation Matrix"]
...
image::barplotpres.png[width=600, height=450, caption="Bar Plot Prescriptions"]
...
# Fill missing values
# Only Total_Patients had some missing values
for df in [X_train, X_val, X_test]:
    df[numeric_cols] = df[numeric_cols].fillna(X_train[numeric_cols].median())  # use training medians
    df[one_hot_cols] = df[one_hot_cols].fillna(0)

# Scale numeric columns using training data only
# fit_tranform calculates the mean and standard deviation from the training data.
# then .tranform use the same mean and std from training (from .fit()) to scale validation and test data.
#Then it applies the transformation (standardization) to X_train.
...
image::roccurvepres.png[width=600, height=450, caption="ROC Curve Prescriptions"]
...
Students should get full credit if they write 1-2 sentences interpreting their results. For example, they may note that the top-scoring prescribers have higher `Cost_per_claim` values and are mostly from the West or South regions. Several belong to the “Primary Care” group, which may indicate they are more likely to prescribe Semaglutide. This aligns with patterns seen earlier in the data where cost, region, and specialty were important predictors.
