...
[NOTE]
====
You do not have to include it as `plt` but it is a common occurrence when writing code. This is similar to how you often see `pd` for `pandas` or `np` for `numpy`. 
====
...
When creating plots, it's important to try to understand the overall trends they reveal. From the plot, we observe that among the first 10 places, Mobile, Juneau, and Little Rock have the highest precipitation levels.
