...
----
combined_df["DATE"] = pd.to_datetime(combined_df["DATE"])
combined_df["DailyAverageDryBulbTemperature"] = pd.to_numeric(
    combined_df["DailyAverageDryBulbTemperature"], errors="coerce")
...
_Explanation:_ Converting `DATE` to a proper datetime type ensures that time-based operations such as resampling, sorting, and indexing work correctly rather than treating dates as plain strings. Coercing `DailyAverageDryBulbTemperature` to numeric replaces any non-numeric entries (like headers re-read as data) with `NaN`, preventing silent errors during aggregation. Sorting by State, Capital, and DATE guarantees each series is in chronological order, which is a strict requirement for all time series methods.
...
cols_to_check = [
    col for col in combined_df.columns
    if col not in ["DATE", "STATION", "State", "Capital"]
]
...
_Explanation:_ Rows with no temperature value provide no useful information for modeling or aggregation and would introduce gaps or biased averages into the weekly resampled series. Removing them before aggregation ensures that weekly means are computed from actual measurements rather than being distorted by blank records.
...
combined_df["WeeklyAvgTemp"] = (
    combined_df
    .groupby(["State", "Capital"])["WeeklyAvgTemp"]
    .ffill()
    .bfill()
)
...
random_states = sample_states(combined_df, n_states=5, seed=42)
random_states
----
...
for (state, capital), group in plot_df.groupby(["State", "Capital"]):
    plt.figure(figsize=(10, 4))
    plt.plot(group["DATE"], group["WeeklyAvgTemp"])
    plt.xlabel("Date")
    plt.ylabel("Weekly Average Dry Bulb Temperature")
    plt.title(f"{capital}, {state}: Weekly Average Dry Bulb Temperature")
    plt.tight_layout()
    plt.show()
----

_Observation:_ All five capitals show a clear seasonal cycle with temperatures peaking in summer and reaching their lowest point in winter, which is consistent with annual weather patterns in the continental U.S. However, the magnitude of seasonal swings and baseline temperature levels differ substantially across locations — warmer southern capitals maintain higher temperatures year-round while northern capitals show more extreme cold in winter months.
...
print("Total unique time series:", n_series)
----

_Explanation:_ Knowing there are 50 unique (State, Capital) time series tells us the total scale of the modeling problem. This count determines how many individual series need to be clustered and later evaluated, and motivates using DTW-based clustering to reduce the number of distinct models that must be fit.
****
...
# Z-score normalize each time series (row-wise)
X = (X - X.mean(axis=1, keepdims=True)) / X.std(axis=1, keepdims=True)
----
...
_Explanation:_ DTW measures the similarity between each pair of normalized time series by finding the optimal alignment of their seasonal patterns, even when peaks and troughs occur at slightly different calendar weeks. K-means then iteratively assigns capitals to the cluster whose centroid is closest in DTW distance, grouping together cities whose temperature shapes are most alike regardless of absolute temperature level.
...
dates = wide_df.columns
wide_values = wide_df.values

for cluster in range(n_clusters):
    plt.figure(figsize=(10, 4))
    for i, label in enumerate(labels):
        if label == cluster:
            plt.plot(dates, wide_values[i], alpha=0.4)
    plt.title(f"DTW Cluster {cluster}")
    plt.xlabel("Date")
...
_Observation:_ The clusters reveal distinct groups of temperature behavior across U.S. capitals. One cluster tends to capture warmer, southern locations that show relatively mild winter dips and high summer peaks, while another groups together colder northern capitals with more extreme seasonal swings. A third cluster may capture moderate or transitional climates, and a fourth may isolate outliers such as Alaska or Hawaii whose temperature profiles differ dramatically from the continental pattern. The separation between clusters confirms that DTW successfully groups capitals by seasonal shape rather than by geographic proximity alone.
****
...
_Explanation:_ Comparing DTW-based temperature clusters to Census regions helps reveal whether climate-driven groupings align with traditional geographic boundaries or tell a different story. If the clusters largely match Census regions, it suggests that geography strongly determines temperature patterns; mismatches would indicate that climate does not simply follow political or historical boundaries and that DTW captures a more nuanced structure.
...
_Observation:_ The DTW clusters show broad geographic coherence, with warmer southern states often grouped together and colder northern states forming a separate cluster, partially mirroring the Census South and Northeast/Midwest regions. However, there are notable mismatches — states in the West that span very different climates (e.g., coastal California vs. mountainous Colorado) may fall into different DTW clusters despite belonging to the same Census region. This suggests that DTW captures climate-driven patterns that cut across traditional regional boundaries.
...
.Solution
****
*5a.* The code is already complete — sort by `["State", "Capital", "DATE"]`, take `g.iloc[:-4]` for training and `g.iloc[-4:]` for the test set. Run as provided.

*5b.* The code is already complete. Run as provided.

_Explanation:_ Averaging z-score-standardized series within each cluster produces a single "prototype" pattern that captures the dominant seasonal shape shared by all members of that cluster, smoothing out city-specific noise. This approach is preferable to modeling each capital independently because it reduces the total number of models from 50 to 4, lowers the risk of overfitting sparse single-city data, and allows information to be pooled across similar cities when estimating seasonal dynamics.

*5c.* The `seasonal_order=(0, 1, 0, 52)` is already filled in the code. Run as provided.

_Explanation:_ In the seasonal order $(P, D, Q, s) = (0, 1, 0, 52)$: $D = 1$ applies one round of seasonal differencing to remove the repeating annual pattern in the standardized series, and $s = 52$ sets the seasonal period to 52 weeks, which corresponds to one full calendar year of weekly observations. Setting $P = Q = 0$ keeps the seasonal component parsimonious — the dominant seasonal signal is captured by differencing alone without additional autoregressive or moving-average seasonal terms.

*5d.* The code is already complete. Run as provided.

_Interpretation:_ An RMSE in the range of a few degrees (on the standardized or original scale) indicates that the cluster-level SARIMAX model is capturing the broad seasonal shape of the four-week test window reasonably well for the best-performing capitals. Higher RMSE values for other capitals suggest that the shared cluster pattern is a less precise fit for those locations, either because the capital's temperature trajectory differs from the cluster average near the end of the year or because the test window falls during a period of high within-cluster variability.

*5e.* The code is already complete. Run as provided.

_Interpretation:_ For well-performing capitals, the dashed predicted line closely tracks the solid observed test line in both level and direction, with peaks and troughs aligning in timing — these capitals have small RMSE values. Poorly performing capitals tend to show a systematic offset (the predicted line is consistently too high or too low) or a timing mismatch where seasonal turning points occur earlier or later in the predictions than in the observations, both of which inflate RMSE and signal that the cluster-average shape is not a precise enough template for those specific locations.
****
...
[source,python]
----
cluster_rmse_summary.sort_values("RMSE")
----

_Interpretation:_ The cluster with the smallest average RMSE is the easiest to forecast — its member capitals share a very consistent and predictable seasonal pattern. The cluster with the largest average RMSE is the hardest to forecast — its members likely have more diverse or irregular temperature trajectories that the single cluster-level model cannot represent well for every capital.

*6c.*

_Reflection:_ Modeling at the DTW cluster level is a reasonable and scalable alternative to fitting 50 individual models — it reduces complexity substantially while still producing forecasts that capture the dominant seasonal shape for most capitals, as reflected in the generally modest RMSE values. To further improve accuracy, one option would be to increase the number of clusters (e.g., from 4 to 6 or 8) so that each group is more internally homogeneous and the shared template is a closer match for every member. A second option would be to fit individual fine-tuning models on top of the cluster forecast for each capital — for example, by modeling the residuals between the cluster prediction and each city's observed values, capturing location-specific deviations that the cluster model misses.
****
