...
We'll begin by showing `argparse` stripped down to its most basic level. The contents of this tutorial were inspired by https://docs.python.org/3/howto/argparse.html[Python's `argparse` tutorial] and https://docs.python.org/3/library/argparse.html[the `argparse` documentation].
