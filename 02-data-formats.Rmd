...
|..|Selects the parent of teh current node|
|@|Selects attributes|
...
In order to make comparisons, we need operators. The following is a list of xml operators: `=`, `!=`, `>`, `<`, `>=`, `<=`, `and`, `or`.
...
`substring-before(str, char)`

`substring-after(str, char)`
