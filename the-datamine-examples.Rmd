...
## How do I delete a file from my RStudio directory?  (asked by Karthik Uppuluri) {#deleting-file-in-RStudio}
...
## How can you run a line of R code in RStudio without clicking the "Run" button? {#running-R-code}
...
##### 2. We've provided you with R code below that will extract a column of our `data.frame` into a vector. What is the 1st value in the vector? What is the 50th value in the vector? What type of data is in the vector? (For this last question, use the `typeof` function to find the type of data.)
...
##### 5. Plot the `max_temp` vector from `myDF`.

```{block, type="bbox"}
**Item(s) to submit:**

- R code used to solve this problem. 
- Plot of the `max_temp` vector from `myDF`.
```
