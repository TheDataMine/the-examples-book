...
1. Setup your SSH keys by following the first 3 steps in the https://the-examples-book.com/starter-guides/tools-and-standards/unix/standard-utilities/ssh[ssh document] in your Anvil terminal. 
...
====
. In the team's directory on Anvil create a GitHub directory for each user.
** This is where the user will work on their code and contribute to the main repository.
** We'll set this up as part of the steps below.
. The GitHub repository should be updated once the code is working and ready for other members of the team to use. 
** The Data Mine team should be able to check your repo and see branches for code in development as well as finalized code in the `Main` branch. 
...
. Find the name of your team's project and run `cd <project_name>`. Replacing the `<project_name>` portion with the name of your team's directory. 
. In this directory run the command `mkdir github_<your_access_id>`. Replacing `<your_access_id>` with the ACCESS ID that you used to log-in to Anvil. 
** This will create a directory that you can use for your GitHub repository. 
. Run the command `cd github_<your_access_id>` to move into the new directory that you just created. 
...
. Navigate back to the terminal on Anvil and run the command `git clone <copied_text>`. Replacing the `<copied_text>` with the `https` link that you copied from the GitHub website. 
...
. In your `github_<your_access_id>` run the `ls` command to see the name of the new repository that you just cloned. 
